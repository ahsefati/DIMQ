...
static void print_version(void)
{
	...
	printf("dimq_ctrl version %s running on libdimq %d.%d.%d.\n", VERSION, major, minor, revision);
}

static void print_usage(void)
{
	printf("dimq_ctrl is a tool for administering certain dimq features.\n");
	print_version();
	printf("\nGeneral usage: dimq_ctrl <module> <module-command> <command-options>\n");
	printf("For module specific help use: dimq_ctrl <module> help\n");
	printf("\nModules available: dynsec\n");
	printf("\nFor more information see:\n");
	printf("    https://dimq.org/man/dimq_ctrl-1.html\n\n");
}


int main(int argc, char *argv[])
{
	struct dimq_ctrl ctrl;
	...
	if(!strcasecmp(argv[0], "dynsec")){
		...
		snprintf(lib_name, sizeof(lib_name), "dimq_ctrl_%s.so", argv[0]);
