...
#endif
static int client_config_line_proc(struct dimq_config *cfg, int *argc, char **argvp[]);


void init_config(struct dimq_config *cfg)
{
	cfg->qos = 1;
...
int ctrl_config_parse(struct dimq_config *cfg, int *argc, char **argv[])
{
	int rc;
	...
	while((*argc) && argv[0][0] == '-'){
		if(!strcmp(argv[0], "-A")){
			...
			if((*argc) == 1){
				...
				if(dimq__parse_socks_url(cfg, argv[1])){
					return 1;
				}
			}
			argv++;
...
static char *get_default_cfg_location(void)
{
	...
	if(env){
		len = strlen(env) + strlen("/dimq_ctrl") + 1;
		loc = malloc(len);
		if(!loc){
			fprintf(stderr, "Error: Out of memory.\n");
			return NULL;
		}
		snprintf(loc, len, "%s/dimq_ctrl", env);
		loc[len-1] = '\0';
	}else{
		env = getenv("HOME");
		if(env){
			len = strlen(env) + strlen("/.config/dimq_ctrl") + 1;
			loc = malloc(len);
			if(!loc){
				fprintf(stderr, "Error: Out of memory.\n");
				return NULL;
			}
			snprintf(loc, len, "%s/.config/dimq_ctrl", env);
			loc[len-1] = '\0';
		}
	}
	...
	if(rc > 0 && rc < 1024){
		len = strlen(env) + strlen("\\dimq_ctrl.conf") + 1;
		loc = malloc(len);
		if(!loc){
			fprintf(stderr, "Error: Out of memory.\n");
			return NULL;
		}
		snprintf(loc, len, "%s\\dimq_ctrl.conf", env);
		loc[len-1] = '\0';
	}
#endif
	return loc;
...
int client_config_load(struct dimq_config *cfg)
{
	int rc;
	FILE *fptr = NULL;
	char line[1024];
...
#  ifdef FINAL_WITH_TLS_PSK
	if(cfg->psk && dimq_tls_psk_set(dimq, cfg->psk, cfg->psk_identity, NULL)){
...
#  endif
	...
	rc = dimq_connect_bind_v5(dimq, cfg->host, port, 60, cfg->bind_address, NULL);
	if(rc>0){
