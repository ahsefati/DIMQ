...
void dynsec__print_usage(void)
{
	...
	printf("    dimq_ctrl dynsec init <new-config-file> <admin-username> [admin-password]\n");
	...
	printf("    https://dimq.org/documentation/dynamic-security/\n\n");
}
...
static void dynsec__payload_callback(struct dimq_ctrl *ctrl, long payloadlen, const void *payload)
{
...
/* dimq_ctrl dynsec init <filename> <admin-user> <admin-password> [role-name] */
static int dynsec_init(int argc, char *argv[])
{
	...
	printf("   dimq_ctrl <connect options> dynsec createClient <username>\n");
	printf("   dimq_ctrl <connect options> dynsec createRole <rolename>\n");
	printf("   dimq_ctrl <connect options> dynsec addRoleACL <rolename> publishClientSend my/topic [priority]\n");
	printf("   dimq_ctrl <connect options> dynsec addClientRole <username> <rolename> [priority]\n");
	printf("See https://dimq.org/documentation/dynamic-security/ for details of all commands.\n");
...
int dynsec__main(int argc, char *argv[], struct dimq_ctrl *ctrl)
{
	int rc = -1;
	cJSON *j_tree;
	...
	if(j_tree == NULL) return dimq_ERR_NOMEM;
