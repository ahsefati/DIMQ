...
#define dimq__malloc(A) malloc((A))
#define dimq__free(A) free((A))
#define _dimq_malloc(A) malloc((A))
#define _dimq_free(A) free((A))
#include <uthash.h>
...
{
	free(chunk->client_id);
	dimq_property_free_all(&chunk->properties);
}
...
int main(int argc, char *argv[])
{
	...
	if(argc == 2){
		...
		fprintf(stderr, "Usage: db_dump [--stats | --client-stats] <dimq db filename>\n");
		return 1;
	}
	...
	if(!memcmp(header, magic, 15)){
		if(do_print) printf("dimq DB dump\n");
		...
		if(db_version > dimq_DB_VERSION){
			if(do_print) printf("Warning: dimq_db_dump does not support this DB version, continuing but expecting errors.\n");
		}

		while(persist__chunk_header_read(fd, &chunk, &length) == dimq_ERR_SUCCESS){
			switch(chunk){
				case DB_CHUNK_CFG:
