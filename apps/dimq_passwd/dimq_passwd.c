...
static enum dimq_pwhash_type hashtype = pw_sha512_pbkdf2;
...
int log__printf(void *dimq, unsigned int level, const char *fmt, ...)
{
	/* Stub for misc_dimq.c */
	UNUSED(dimq);
	UNUSED(level);
	UNUSED(fmt);
	return 0;
...
{
	printf("dimq_passwd is a tool for managing password files for dimq.\n\n");
	printf("Usage: dimq_passwd [-H sha512 | -H sha512-pbkdf2] [-c | -D] passwordfile username\n");
	printf("       dimq_passwd [-H sha512 | -H sha512-pbkdf2] [-c] -b passwordfile username password\n");
	printf("       dimq_passwd -U passwordfile\n");
	...
	printf("      dimq 1.6 and earlier defaulted to sha512.\n");
