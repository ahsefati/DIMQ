...
Contributors:
   ...
   Amirhossein Sefati - Add some functionalities required for DIMQ.
*/
...
int my_publish(struct dimq *dimq, int *mid, const char *topic, int payloadlen, void *payload, int qos, bool retain)
{
	...
	if(cfg.protocol_version == MQTT_PROTOCOL_V5 && cfg.have_topic_alias && first_publish == false){
		return dimq_publish_v5(dimq, mid, NULL, payloadlen, payload, qos, retain, cfg.publish_props);
	}else{
		first_publish = false;
	...
	if(!result){
		switch(cfg.pub_mode){
			...
			case MSGMODE_STDIN_FILE:
				rc = my_publish(dimq, &mid_sent, cfg.topic, cfg.msglen, cfg.message, cfg.qos, cfg.retain);
				break;
			case MSGMODE_NULL:
				rc = my_publish(dimq, &mid_sent, cfg.topic, 0, NULL, cfg.qos, cfg.retain);
				break;
		...
		if(rc){
			switch(rc){
				...
				case dimq_ERR_PAYLOAD_SIZE:
					...
					break;
				case dimq_ERR_QOS_NOT_SUPPORTED:
...
void my_publish_callback(struct dimq *dimq, void *obj, int mid, int reason_code, const dimq_property *properties)
{
	...
	last_mid_sent = mid;
	if(reason_code > 127){
		err_printf(&cfg, "Warning: Publish %d failed: %s.\n", mid, dimq_reason_string(reason_code));
		dimq_property_read_string(properties, MQTT_PROP_REASON_STRING, &reason_string, false);
		if(reason_string){
			err_printf(&cfg, "%s\n", reason_string);
			free(reason_string);
		}
...
static int pub_stdin_line_loop(struct dimq *dimq)
{
	char *buf2;
		...
		if(status == STATUS_CONNACK_RECVD){
			...
			while(status == STATUS_CONNACK_RECVD && fgets(&line_buf[pos], read_len, stdin)){
				...
				if(line_buf[buf_len_actual-1] == '\n'){
					...
					rc = my_publish(dimq, &mid_sent, cfg.topic, buf_len_actual-1, line_buf, cfg.qos, cfg.retain);
					pos = 0;
					if(rc != dimq_ERR_SUCCESS && rc != dimq_ERR_NO_CONN){
						return rc;
					}
					break;
				}else{
			...
			if(pos != 0){
				rc = my_publish(dimq, &mid_sent, cfg.topic, buf_len_actual, line_buf, cfg.qos, cfg.retain);
				if(rc){
...
static int pub_other_loop(struct dimq *dimq)
{
	int rc;
	int loop_delay = 1000;
	...
	do{
		rc = dimq_loop(dimq, loop_delay, 1);
		if(ready_for_repeat && check_repeat_time()){
			...
			if(rc != dimq_ERR_SUCCESS && rc != dimq_ERR_NO_CONN){
				err_printf(&cfg, "Error sending repeat publish: %s", dimq_strerror(rc));
			}
		}
	}while(rc == dimq_ERR_SUCCESS);
...
{
	if(cfg.pub_mode == MSGMODE_STDIN_LINE){
		return pub_stdin_line_loop(dimq);
	}else{
		return pub_other_loop(dimq);
	}
}
...
static void print_version(void)
{
	...
	printf("dimq_pub version %s running on libdimq %d.%d.%d.\n", VERSION, major, minor, revision);
}
...
static void print_usage(void)
{
	...
	dimq_lib_version(&major, &minor, &revision);
	printf("dimq_pub is a simple mqtt client that will publish a message on a single topic and exit.\n");
	printf("dimq_pub version %s running on libdimq %d.%d.%d.\n\n", VERSION, major, minor, revision);
	printf("Usage: dimq_pub {[-h host] [--unix path] [-p port] [-u username] [-P password] -t topic | -L URL}\n");
	...
	printf(" -i : id to use for this client. Defaults to dimq_pub_ appended with the process id.\n");
...
int main(int argc, char *argv[])
{
	...
	if(rc){
		if(rc == 2){
			...
			fprintf(stderr, "\nUse 'dimq_pub --help' to see usage.\n");
		}
		goto cleanup;
	}
	...
	dimq = dimq_new(cfg.id, cfg.clean_session, NULL);
	if(!dimq){
		switch(errno){
			case ENOMEM:
				err_printf(&cfg, "Error: Out of memory.\n");
	...
	if(cfg.debug){
		dimq_log_callback_set(dimq, my_log_callback);
	}
	dimq_connect_v5_callback_set(dimq, my_connect_callback);
	dimq_disconnect_v5_callback_set(dimq, my_disconnect_callback);
	dimq_publish_v5_callback_set(dimq, my_publish_callback);

	if(client_opts_set(dimq, &cfg)){
		goto cleanup;
	}

	rc = client_connect(dimq, &cfg);
	if(rc){
		goto cleanup;
	}

	rc = pub_shared_loop(dimq);
