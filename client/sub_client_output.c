...
#ifdef WITH_CJSON
static int json_print_properties(cJSON *root, const dimq_property *properties)
{
	int identifier;
	uint8_t i8value = 0;
	uint16_t i16value = 0;
	...
	for(prop=properties; prop != NULL; prop = dimq_property_next(prop)){
		tmp = NULL;
		identifier = dimq_property_identifier(prop);
		switch(identifier){
			case MQTT_PROP_PAYLOAD_FORMAT_INDICATOR:
				dimq_property_read_byte(prop, MQTT_PROP_PAYLOAD_FORMAT_INDICATOR, &i8value, false);
				tmp = cJSON_CreateNumber(i8value);
				break;

			case MQTT_PROP_MESSAGE_EXPIRY_INTERVAL:
				dimq_property_read_int32(prop, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, &i32value, false);
				tmp = cJSON_CreateNumber(i32value);
				break;

			case MQTT_PROP_CONTENT_TYPE:
			case MQTT_PROP_RESPONSE_TOPIC:
				dimq_property_read_string(prop, identifier, &strvalue, false);
				if(strvalue == NULL) return dimq_ERR_NOMEM;
			...
			case MQTT_PROP_CORRELATION_DATA:
				dimq_property_read_binary(prop, MQTT_PROP_CORRELATION_DATA, (void **)&binvalue, &i16value, false);
				if(binvalue == NULL) return dimq_ERR_NOMEM;
			...
			case MQTT_PROP_SUBSCRIPTION_IDENTIFIER:
				dimq_property_read_varint(prop, MQTT_PROP_SUBSCRIPTION_IDENTIFIER, &i32value, false);
				tmp = cJSON_CreateNumber(i32value);
				break;

			case MQTT_PROP_TOPIC_ALIAS:
				dimq_property_read_int16(prop, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, &i16value, false);
			...
			case MQTT_PROP_USER_PROPERTY:
				...
				dimq_property_read_string_pair(prop, MQTT_PROP_USER_PROPERTY, &strname, &strvalue, false);
				if(strname == NULL || strvalue == NULL) return dimq_ERR_NOMEM;
		...
		if(tmp != NULL){
			cJSON_AddItemToObject(prop_json, dimq_property_identifier_to_string(identifier), tmp);
		}
	}
...
static int json_print(const struct dimq_message *message, const dimq_property *properties, const struct tm *ti, int ns, bool escaped, bool pretty)
{
	char buf[100];
#ifdef WITH_CJSON
	cJSON *root;
...
static void formatted_print_percent(const struct dimq_config *lcfg, const struct dimq_message *message, const dimq_property *properties, char format, char align, char pad, int field_width, int precision)
{
	struct tm *ti = NULL;
	long ns = 0;
	char buf[100];
	...
	switch(format){
		...
		case 'A':
			if(dimq_property_read_int16(properties, MQTT_PROP_TOPIC_ALIAS, &i16value, false)){
		...
		case 'C':
			if(dimq_property_read_string(properties, MQTT_PROP_CONTENT_TYPE, &strvalue, false)){
		...
		case 'D':
			if(dimq_property_read_binary(properties, MQTT_PROP_CORRELATION_DATA, (void **)&binvalue, &i16value, false)){
		...
		case 'E':
			if(dimq_property_read_int32(properties, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, &i32value, false)){
		...
		case 'F':
			if(dimq_property_read_byte(properties, MQTT_PROP_PAYLOAD_FORMAT_INDICATOR, &i8value, false)){
		...
		case 'j':
			...
			if(json_print(message, properties, ti, (int)ns, true, lcfg->pretty) != dimq_ERR_SUCCESS){
		...
		case 'P':
			strname = NULL;
			strvalue = NULL;
			prop = dimq_property_read_string_pair(properties, MQTT_PROP_USER_PROPERTY, &strname, &strvalue, false);
			while(prop){
				...
				prop = dimq_property_read_string_pair(prop, MQTT_PROP_USER_PROPERTY, &strname, &strvalue, true);
				if(prop){
					fputc(' ', stdout);
				}
			}
		...
		case 'R':
			if(dimq_property_read_string(properties, MQTT_PROP_RESPONSE_TOPIC, &strvalue, false)){
				formatted_print_str(strvalue, align, field_width, precision);
				free(strvalue);
			}
			break;
		...
		case 'S':
			if(dimq_property_read_varint(properties, MQTT_PROP_SUBSCRIPTION_IDENTIFIER, &i32value, false)){
				formatted_print_int((int)i32value, align, pad, field_width);
			}else{
				formatted_print_blank(pad, field_width);
			}
...
static void formatted_print(const struct dimq_config *lcfg, const struct dimq_message *message, const dimq_property *properties)
{
	size_t len;
	size_t i;
	struct tm *ti = NULL;
...
void print_message(struct dimq_config *lcfg, const struct dimq_message *message, const dimq_property *properties)
{
#ifdef WIN32
	unsigned int r = 0;
#else
