...
static void my_message_callback(struct dimq *dimq, void *obj, const struct dimq_message *message, const dimq_property *properties)
{
	...
	if(cfg.remove_retained && message->retain){
		dimq_publish(dimq, &last_mid, message->topic, 0, NULL, 1, true);
	}
...
static void my_connect_callback(struct dimq *dimq, void *obj, int result, int flags, const dimq_property *properties)
{
	int i;
	...
	if(!result){
		dimq_subscribe_multiple(dimq, NULL, cfg.topic_count, cfg.topics, cfg.qos, cfg.sub_opts, cfg.subscribe_props);

		for(i=0; i<cfg.unsub_topic_count; i++){
			dimq_unsubscribe_v5(dimq, NULL, cfg.unsub_topics[i], cfg.unsubscribe_props);
		}
	}else{
		if(result){
...
static void my_log_callback(struct dimq *dimq, void *obj, int level, const char *str)
{
	UNUSED(dimq);
	UNUSED(obj);
	UNUSED(level);
...
static void print_version(void)
{
	...
	printf("dimq_sub version %s running on libdimq %d.%d.%d.\n", VERSION, major, minor, revision);
}
...
static void print_usage(void)
{
	...
	dimq_lib_version(&major, &minor, &revision);
	printf("dimq_sub is a simple mqtt client that will subscribe to a set of topics and print all messages it receives.\n");
	printf("dimq_sub version %s running on libdimq %d.%d.%d.\n\n", VERSION, major, minor, revision);
	printf("Usage: dimq_sub {[-h host] [--unix path] [-p port] [-u username] [-P password] -t topic | -L URL [-t topic]}\n");
	...
	printf(" -i : id to use for this client. Defaults to dimq_sub_ appended with the process id.\n");
...
int main(int argc, char *argv[])
{
	...
	if(rc){
		if(rc == 2){
			...
			fprintf(stderr, "\nUse 'dimq_sub --help' to see usage.\n");
		}
		goto cleanup;
	}
	...
	rc = dimq_loop_forever(g_dimq, -1, 1);
