...
static void my_signal_handler(int signum)
{
	if(signum == SIGALRM){
		...
		dimq_disconnect_v5(g_dimq, MQTT_RC_DISCONNECT_WITH_WILL_MSG, cfg.disconnect_props);
		timed_out = true;
	}
}
#endif
...
int my_publish(struct dimq *dimq, int *mid, const char *topic, int payloadlen, void *payload, int qos, bool retain)
{
	if(cfg.protocol_version < MQTT_PROTOCOL_V5){
		...
		return dimq_publish_v5(dimq, mid, topic, payloadlen, payload, qos, retain, cfg.publish_props);
	}
}


static void my_message_callback(struct dimq *dimq, void *obj, const struct dimq_message *message, const dimq_property *properties)
{
	UNUSED(dimq);
	UNUSED(obj);
	UNUSED(properties);
	...
	if(cfg.retained_only && !message->retain && process_messages){
		process_messages = false;
		dimq_disconnect_v5(dimq, 0, cfg.disconnect_props);
		return;
	}
	...
	if(cfg.filter_outs){
		for(i=0; i<cfg.filter_out_count; i++){
			dimq_topic_matches_sub(cfg.filter_outs[i], message->topic, &res);
			if(res) return;
		}
	}
...
void my_connect_callback(struct dimq *dimq, void *obj, int result, int flags, const dimq_property *properties)
{
	...
	if(!result){
		...
		dimq_subscribe_v5(dimq, NULL, cfg.response_topic, cfg.qos, 0, cfg.subscribe_props);
	}else{
		client_state = rr_s_disconnect;
		if(result){
			if(result == MQTT_RC_UNSUPPORTED_PROTOCOL_VERSION){
				err_printf(&cfg, "Connection error: %s. dimq_rr only supports connecting to an MQTT v5 broker\n", dimq_reason_string(result));
			}else{
...
static void my_subscribe_callback(struct dimq *dimq, void *obj, int mid, int qos_count, const int *granted_qos)
{
	UNUSED(obj);
	UNUSED(mid);
	UNUSED(qos_count);
...
static void print_version(void)
{
	...
	dimq_lib_version(&major, &minor, &revision);
	printf("dimq_rr version %s running on libdimq %d.%d.%d.\n", VERSION, major, minor, revision);
}
...
static void print_usage(void)
{
	...
	dimq_lib_version(&major, &minor, &revision);
	printf("dimq_rr is an mqtt client that can be used to publish a request message and wait for a response.\n");
	...
	printf("dimq_rr version %s running on libdimq %d.%d.%d.\n\n", VERSION, major, minor, revision);
	printf("Usage: dimq_rr {[-h host] [--unix path] [-p port] [-u username] [-P password] -t topic | -L URL} -e response-topic\n");
	...
	printf(" -i : id to use for this client. Defaults to dimq_rr_ appended with the process id.\n");
...
int main(int argc, char *argv[])
{
	...
	rc = dimq_property_add_string(&cfg.publish_props, MQTT_PROP_RESPONSE_TOPIC, cfg.response_topic);
	if(rc){
	...
	g_dimq = dimq_new(cfg.id, cfg.clean_session, &cfg);
	if(!g_dimq){
		switch(errno){
			case ENOMEM:
	...
	if(client_opts_set(g_dimq, &cfg)){
		goto cleanup;
	}
	if(cfg.debug){
		dimq_log_callback_set(g_dimq, my_log_callback);
	}
	dimq_connect_v5_callback_set(g_dimq, my_connect_callback);
	dimq_subscribe_callback_set(g_dimq, my_subscribe_callback);
	dimq_message_v5_callback_set(g_dimq, my_message_callback);

	rc = client_connect(g_dimq, &cfg);
	if(rc){
		goto cleanup;
	}
	...
	do{
		...
		if(client_state == rr_s_ready_to_publish){
			...
			switch(cfg.pub_mode){
				...
				case MSGMODE_FILE:
				case MSGMODE_STDIN_FILE:
					rc = my_publish(g_dimq, &mid_sent, cfg.topic, cfg.msglen, cfg.message, cfg.qos, cfg.retain);
					break;
				case MSGMODE_NULL:
					rc = my_publish(g_dimq, &mid_sent, cfg.topic, 0, NULL, cfg.qos, cfg.retain);
					break;
				case MSGMODE_STDIN_LINE:
					/* FIXME */
					break;
	...
	}while(rc == dimq_ERR_SUCCESS && client_state != rr_s_disconnect);

	dimq_destroy(g_dimq);
	dimq_lib_cleanup();

	if(cfg.msg_count>0 && rc == dimq_ERR_NO_CONN){
		rc = 0;
	}
	client_config_cleanup(&cfg);
	if(timed_out){
		err_printf(&cfg, "Timed out\n");
		return dimq_ERR_TIMEOUT;
	}else if(rc){
