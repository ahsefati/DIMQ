...
#ifdef WITH_SOCKS
static int dimq__parse_socks_url(struct dimq_config *cfg, char *url);
#endif
static int client_config_line_proc(struct dimq_config *cfg, int pub_or_sub, int argc, char *argv[]);
...
static void init_config(struct dimq_config *cfg, int pub_or_sub)
{
...
void client_config_cleanup(struct dimq_config *cfg)
{
	int i;
	free(cfg->id);
	free(cfg->id_prefix);
...
#endif
	dimq_property_free_all(&cfg->connect_props);
	dimq_property_free_all(&cfg->publish_props);
	dimq_property_free_all(&cfg->subscribe_props);
	dimq_property_free_all(&cfg->unsubscribe_props);
	dimq_property_free_all(&cfg->disconnect_props);
	dimq_property_free_all(&cfg->will_props);
}

int client_config_load(struct dimq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	int rc;
	FILE *fptr;
	char line[1024];
	...
	if(env){
		len = strlen(env) + strlen("/dimq_pub") + 1;
		loc = malloc(len);
		if(!loc){
		...
		if(pub_or_sub == CLIENT_PUB){
			snprintf(loc, len, "%s/dimq_pub", env);
		}else if(pub_or_sub == CLIENT_SUB){
			snprintf(loc, len, "%s/dimq_sub", env);
		}else{
			snprintf(loc, len, "%s/dimq_rr", env);
		}
		loc[len-1] = '\0';
	}else{
		env = getenv("HOME");
		if(env){
			len = strlen(env) + strlen("/.config/dimq_pub") + 1;
			loc = malloc(len);
			if(!loc){
				err_printf(cfg, "Error: Out of memory.\n");
				return 1;
			...
			if(pub_or_sub == CLIENT_PUB){
				snprintf(loc, len, "%s/.config/dimq_pub", env);
			}else if(pub_or_sub == CLIENT_SUB){
				snprintf(loc, len, "%s/.config/dimq_sub", env);
			}else{
				snprintf(loc, len, "%s/.config/dimq_rr", env);
			}
			loc[len-1] = '\0';
		}
	}
	...
	if(rc > 0 && rc < 1024){
		len = strlen(env) + strlen("\\dimq_pub.conf") + 1;
		loc = malloc(len);
		if(!loc){
			err_printf(cfg, "Error: Out of memory.\n");
			return 1;
		...
		if(pub_or_sub == CLIENT_PUB){
			snprintf(loc, len, "%s\\dimq_pub.conf", env);
		}else if(pub_or_sub == CLIENT_SUB){
			snprintf(loc, len, "%s\\dimq_sub.conf", env);
		}else{
			snprintf(loc, len, "%s\\dimq_rr.conf", env);
		}
		loc[len-1] = '\0';
	}
#endif
	...
	if(cfg->protocol_version == 5){
		...
		if(cfg->session_expiry_interval > 0){
			...
			rc = dimq_property_add_int32(&cfg->connect_props, MQTT_PROP_SESSION_EXPIRY_INTERVAL, (uint32_t )cfg->session_expiry_interval);
			if(rc){
	...
	rc = dimq_property_check_all(CMD_CONNECT, cfg->connect_props);
	if(rc){
		err_printf(cfg, "Error in CONNECT properties: %s\n", dimq_strerror(rc));
		return 1;
	}
	rc = dimq_property_check_all(CMD_PUBLISH, cfg->publish_props);
	if(rc){
		err_printf(cfg, "Error in PUBLISH properties: %s\n", dimq_strerror(rc));
		return 1;
	}
	rc = dimq_property_check_all(CMD_SUBSCRIBE, cfg->subscribe_props);
	if(rc){
		err_printf(cfg, "Error in SUBSCRIBE properties: %s\n", dimq_strerror(rc));
		return 1;
	}
	rc = dimq_property_check_all(CMD_UNSUBSCRIBE, cfg->unsubscribe_props);
	if(rc){
		err_printf(cfg, "Error in UNSUBSCRIBE properties: %s\n", dimq_strerror(rc));
		return 1;
	}
	rc = dimq_property_check_all(CMD_DISCONNECT, cfg->disconnect_props);
	if(rc){
		err_printf(cfg, "Error in DISCONNECT properties: %s\n", dimq_strerror(rc));
		return 1;
	}
	rc = dimq_property_check_all(CMD_WILL, cfg->will_props);
	if(rc){
...
static int cfg_add_topic(struct dimq_config *cfg, int type, char *topic, const char *arg)
{
	if(dimq_validate_utf8(topic, (int )strlen(topic))){
		fprintf(stderr, "Error: Malformed UTF-8 in %s argument.\n\n", arg);
		return 1;
	}
	if(type == CLIENT_PUB || type == CLIENT_RR){
		if(dimq_pub_topic_check(topic) == dimq_ERR_INVAL){
		...
		if(dimq_sub_topic_check(topic) == dimq_ERR_INVAL){
...
int client_config_line_proc(struct dimq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	int i;
	int tmpi;
	float f;
	...
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-A")){
			...
			if(i==argc-1){
				...
				if(dimq__parse_socks_url(cfg, argv[i+1])){
					return 1;
				}
				i++;
			}
			...
			if(i==argc-1){
				...
				if(dimq_validate_utf8(argv[i+1], (int )strlen(argv[i+1]))){
					fprintf(stderr, "Error: Malformed UTF-8 in -T argument.\n\n");
					return 1;
				}
				if(dimq_sub_topic_check(argv[i+1]) == dimq_ERR_INVAL){
			...
			if(i==argc-1){
				...
				if(dimq_pub_topic_check(argv[i+1]) == dimq_ERR_INVAL){
...
int client_opts_set(struct dimq *dimq, struct dimq_config *cfg)
{
#if defined(WITH_TLS) || defined(WITH_SOCKS)
	int rc;
#endif

	dimq_int_option(dimq, dimq_OPT_PROTOCOL_VERSION, cfg->protocol_version);

	if(cfg->will_topic && dimq_will_set_v5(dimq, cfg->will_topic,
	...
	if((cfg->username || cfg->password) && dimq_username_pw_set(dimq, cfg->username, cfg->password)){
	...
	if(cfg->cafile || cfg->capath){
		rc = dimq_tls_set(dimq, cfg->cafile, cfg->capath, cfg->certfile, cfg->keyfile, NULL);
		if(rc){
...
#  ifdef FINAL_WITH_TLS_PSK
		...
		if(dimq_tls_psk_set(dimq, cfg->psk, cfg->psk_identity, NULL)){
...
#  endif
		...
		dimq_int_option(dimq, dimq_OPT_TLS_USE_OS_CERTS, 1);
	}
	if(cfg->tls_use_os_certs){
		dimq_int_option(dimq, dimq_OPT_TLS_USE_OS_CERTS, 1);
	}

	if(cfg->insecure && dimq_tls_insecure_set(dimq, true)){
		err_printf(cfg, "Error: Problem setting TLS insecure option.\n");
		dimq_lib_cleanup();
		return 1;
	}
	if(cfg->tls_engine && dimq_string_option(dimq, dimq_OPT_TLS_ENGINE, cfg->tls_engine)){
		err_printf(cfg, "Error: Problem setting TLS engine, is %s a valid engine?\n", cfg->tls_engine);
		dimq_lib_cleanup();
		return 1;
	}
	if(cfg->keyform && dimq_string_option(dimq, dimq_OPT_TLS_KEYFORM, cfg->keyform)){
		err_printf(cfg, "Error: Problem setting key form, it must be one of 'pem' or 'engine'.\n");
		dimq_lib_cleanup();
		return 1;
	}
	if(cfg->tls_engine_kpass_sha1 && dimq_string_option(dimq, dimq_OPT_TLS_ENGINE_KPASS_SHA1, cfg->tls_engine_kpass_sha1)){
		err_printf(cfg, "Error: Problem setting TLS engine key pass sha, is it a 40 character hex string?\n");
		dimq_lib_cleanup();
		return 1;
	}
	if(cfg->tls_alpn && dimq_string_option(dimq, dimq_OPT_TLS_ALPN, cfg->tls_alpn)){
		err_printf(cfg, "Error: Problem setting TLS ALPN protocol.\n");
		dimq_lib_cleanup();
		return 1;
	}
	if((cfg->tls_version || cfg->ciphers) && dimq_tls_opts_set(dimq, 1, cfg->tls_version, cfg->ciphers)){
		err_printf(cfg, "Error: Problem setting TLS options, check the options are valid.\n");
		dimq_lib_cleanup();
		return 1;
	}
#endif
	dimq_max_inflight_messages_set(dimq, cfg->max_inflight);
#ifdef WITH_SOCKS
	if(cfg->socks5_host){
		rc = dimq_socks5_set(dimq, cfg->socks5_host, cfg->socks5_port, cfg->socks5_username, cfg->socks5_password);
		if(rc){
			dimq_lib_cleanup();
			return rc;
		}
...
#endif
	if(cfg->tcp_nodelay){
		dimq_int_option(dimq, dimq_OPT_TCP_NODELAY, 1);
	}
	...
	if(cfg->msg_count > 0 && cfg->msg_count < 20){
		 ...
		 * before we quit.*/
		dimq_int_option(dimq, dimq_OPT_RECEIVE_MAXIMUM, cfg->msg_count);
	}
	return dimq_ERR_SUCCESS;
}

int client_id_generate(struct dimq_config *cfg)
{
	if(cfg->id_prefix){
...
int client_connect(struct dimq *dimq, struct dimq_config *cfg)
{
#ifndef WIN32
	char *err;
#else
...
#ifdef WITH_SRV
	if(cfg->use_srv){
		rc = dimq_connect_srv(dimq, cfg->host, cfg->keepalive, cfg->bind_address);
	}else{
		rc = dimq_connect_bind_v5(dimq, cfg->host, port, cfg->keepalive, cfg->bind_address, cfg->connect_props);
	}
#else
	rc = dimq_connect_bind_v5(dimq, cfg->host, port, cfg->keepalive, cfg->bind_address, cfg->connect_props);
#endif
	if(rc>0){
		if(rc == dimq_ERR_ERRNO){
#ifndef WIN32
			err = strerror(errno);
#else
...
#ifdef WITH_SOCKS
/* Convert %25 -> %, %3a, %3A -> :, %40 -> @ */
static int dimq__urldecode(char *str)
{
	size_t i, j;
	size_t len;
	if(!str) return 0;
...
static int dimq__parse_socks_url(struct dimq_config *cfg, char *url)
{
	...
	if(dimq__urldecode(username)){
		goto cleanup;
	}
	if(dimq__urldecode(password)){
		goto cleanup;
	}
	if(port){
		port_int = atoi(port);
...
void err_printf(const struct dimq_config *cfg, const char *fmt, ...)
{
	va_list va;
