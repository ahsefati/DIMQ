...
int cfg_parse_property(struct dimq_config *cfg, int argc, char *argv[], int *idx)
{
	char *cmdname = NULL, *propname = NULL;
	char *key = NULL, *value = NULL;
	int cmd, identifier, type;
	dimq_property **proplist;
	int rc;
	long tmpl;
	size_t szt;
	...
	cmdname = argv[*idx];
	if(dimq_string_to_command(cmdname, &cmd)){
		fprintf(stderr, "Error: Invalid command given in --property argument.\n\n");
		return dimq_ERR_INVAL;
	}

	propname = argv[(*idx)+1];
	if(dimq_string_to_property_info(propname, &identifier, &type)){
		fprintf(stderr, "Error: Invalid property name given in --property argument.\n\n");
		return dimq_ERR_INVAL;
	}

	if(dimq_property_check_command(cmd, identifier)){
	...
	switch(type){
		case MQTT_PROP_TYPE_BYTE:
			...
			rc = dimq_property_add_byte(proplist, identifier, (uint8_t )tmpl);
			break;
		case MQTT_PROP_TYPE_INT16:
			...
			rc = dimq_property_add_int16(proplist, identifier, (uint16_t )tmpl);
			break;
		case MQTT_PROP_TYPE_INT32:
			...
			rc = dimq_property_add_int32(proplist, identifier, (uint32_t )tmpl);
			break;
		case MQTT_PROP_TYPE_VARINT:
			...
			rc = dimq_property_add_varint(proplist, identifier, (uint32_t )tmpl);
			break;
		case MQTT_PROP_TYPE_BINARY:
			...
			rc = dimq_property_add_binary(proplist, identifier, value, (uint16_t )szt);
			break;
		case MQTT_PROP_TYPE_STRING:
			rc = dimq_property_add_string(proplist, identifier, value);
			break;
