...
typedef int (*dimq_FUNC_acl_check)(struct dimq_evt_acl_check *, struct dynsec__rolelist *);
...
static int acl_check_publish_c_recv(struct dimq_evt_acl_check *ed, struct dynsec__rolelist *base_rolelist)
{
	...
	HASH_ITER(hh, base_rolelist, rolelist, rolelist_tmp){
		HASH_ITER(hh, rolelist->role->acls.publish_c_recv, acl, acl_tmp){
			dimq_topic_matches_sub(acl->topic, ed->topic, &result);
			if(result){
				if(acl->allow){
...
static int acl_check_publish_c_send(struct dimq_evt_acl_check *ed, struct dynsec__rolelist *base_rolelist)
{
	struct dynsec__rolelist *rolelist, *rolelist_tmp = NULL;
	struct dynsec__acl *acl, *acl_tmp = NULL;
	bool result;
...
static int acl_check_subscribe(struct dimq_evt_acl_check *ed, struct dynsec__rolelist *base_rolelist)
{
	struct dynsec__rolelist *rolelist, *rolelist_tmp = NULL;
	struct dynsec__acl *acl, *acl_tmp = NULL;
	size_t len;
...
static int acl_check_unsubscribe(struct dimq_evt_acl_check *ed, struct dynsec__rolelist *base_rolelist)
{
	struct dynsec__rolelist *rolelist, *rolelist_tmp = NULL;
	struct dynsec__acl *acl, *acl_tmp = NULL;
	size_t len;
...
static int acl_check(struct dimq_evt_acl_check *ed, dimq_FUNC_acl_check check, bool acl_default_access)
{
	...
	username = dimq_client_username(ed->client);

	if(username){
		client = dynsec_clients__find(username);
		if(client == NULL) return dimq_ERR_PLUGIN_DEFER;
