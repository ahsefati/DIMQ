...
static void send_response(cJSON *tree)
{
	...
	dimq_broker_publish(NULL, "$CONTROL/dynamic-security/v1/response",
...
static int dynsec_control_callback(int event, void *event_data, void *userdata)
{
	...
	/* Parse cJSON tree.
	 * Using cJSON_ParseWithLength() is the best choice here, but dimq
...
static int dynsec__process_set_default_acl_access(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	cJSON_ArrayForEach(j_action, j_actions){
		...
		if(j_acltype && cJSON_IsString(j_acltype)
			...
			dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | setDefaultACLAccess | acltype=%s | allow=%s",
...
static int dynsec__process_get_default_acl_access(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | getDefaultACLAccess",
...
static int dynsec__config_load(void)
{
	...
	if(fptr == NULL){
		dimq_log_printf(dimq_LOG_ERR, "Error loading Dynamic security plugin config: File is not readable - check permissions.\n");
		return 1;
	}
	...
	flen_l = ftell(fptr);
	if(flen_l < 0){
		dimq_log_printf(dimq_LOG_ERR, "Error loading Dynamic security plugin config: %s\n", strerror(errno));
		fclose(fptr);
		return 1;
	}else if(flen_l == 0){
		fclose(fptr);
	...
	json_str = dimq_calloc(flen+1, sizeof(char));
	if(json_str == NULL){
		dimq_log_printf(dimq_LOG_ERR, "Error: Out of memory.");
		fclose(fptr);
		return 1;
	}
	if(fread(json_str, 1, flen, fptr) != flen){
		dimq_log_printf(dimq_LOG_WARNING, "Error loading Dynamic security plugin config: Unable to read file contents.\n");
		dimq_free(json_str);
		fclose(fptr);
		return 1;
	}
	...
	dimq_free(json_str);
	if(tree == NULL){
		dimq_log_printf(dimq_LOG_ERR, "Error loading Dynamic security plugin config: File is not valid JSON.\n");
		return 1;
	}
...
void dynsec__config_save(void)
{
	...
	if(json_str == NULL){
		cJSON_Delete(tree);
		dimq_log_printf(dimq_LOG_ERR, "Error saving Dynamic security plugin config: Out of memory.\n");
		return;
	}
	cJSON_Delete(tree);
	...
	file_path = dimq_malloc(file_path_len);
	if(file_path == NULL){
		dimq_free(json_str);
		dimq_log_printf(dimq_LOG_ERR, "Error saving Dynamic security plugin config: Out of memory.\n");
		return;
	...
	if(fptr == NULL){
		dimq_free(json_str);
		dimq_free(file_path);
		dimq_log_printf(dimq_LOG_ERR, "Error saving Dynamic security plugin config: File is not writable - check permissions.\n");
		return;
	}
	...
	if(rename(file_path, config_file) < 0){
		dimq_log_printf(dimq_LOG_ERR, "Error updating dynsec config file: %s", strerror(errno));
	}
	dimq_free(file_path);
}


int dimq_plugin_init(dimq_plugin_id_t *identifier, void **user_data, struct dimq_opt *options, int option_count)
{
	int i;
	...
	for(i=0; i<option_count; i++){
		if(!strcasecmp(options[i].key, "config_file")){
			config_file = dimq_strdup(options[i].value);
			if(config_file == NULL){
				return dimq_ERR_NOMEM;
			}
			break;
	...
	if(config_file == NULL){
		dimq_log_printf(dimq_LOG_WARNING, "Warning: Dynamic security plugin has no plugin_opt_config_file defined. The plugin will not be activated.");
		return dimq_ERR_SUCCESS;
	}
	...
	dynsec__config_load();
	dimq_callback_register(plg_id, dimq_EVT_CONTROL, dynsec_control_callback, "$CONTROL/dynamic-security/v1", NULL);
	dimq_callback_register(plg_id, dimq_EVT_BASIC_AUTH, dynsec_auth__basic_auth_callback, NULL, NULL);
	dimq_callback_register(plg_id, dimq_EVT_ACL_CHECK, dynsec__acl_check_callback, NULL, NULL);

	return dimq_ERR_SUCCESS;
}

int dimq_plugin_cleanup(void *user_data, struct dimq_opt *options, int option_count)
{
	UNUSED(user_data);
	UNUSED(options);
	UNUSED(option_count);
	...
	if(plg_id){
		dimq_callback_unregister(plg_id, dimq_EVT_CONTROL, dynsec_control_callback, "$CONTROL/dynamic-security/v1");
		dimq_callback_unregister(plg_id, dimq_EVT_BASIC_AUTH, dynsec_auth__basic_auth_callback, NULL);
		dimq_callback_unregister(plg_id, dimq_EVT_ACL_CHECK, dynsec__acl_check_callback, NULL);
	}
	dynsec_groups__cleanup();
	dynsec_clients__cleanup();
	dynsec_roles__cleanup();

	dimq_free(config_file);
	config_file = NULL;
	return dimq_ERR_SUCCESS;
}
...
int dynsec__handle_control(cJSON *j_responses, struct dimq *context, cJSON *commands)
{
	...
	cJSON_ArrayForEach(aiter, commands){
		if(cJSON_IsObject(aiter)){
			if(json_get_string(aiter, "command", &command, false) == dimq_ERR_SUCCESS){
				if(json_get_string(aiter, "correlationData", &correlation_data, true) != dimq_ERR_SUCCESS){
