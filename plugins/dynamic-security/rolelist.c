...
static int dynsec_rolelist__add(struct dynsec__rolelist **base_rolelist, struct dynsec__role *role, int priority)
{
	...
	if(role == NULL) return dimq_ERR_INVAL;

	HASH_FIND(hh, *base_rolelist, role->rolename, strlen(role->rolename), rolelist);
	if(rolelist){
		return dimq_ERR_ALREADY_EXISTS;
	}else{
		rolelist = dimq_calloc(1, sizeof(struct dynsec__rolelist));
		if(rolelist == NULL) return dimq_ERR_NOMEM;

		rolelist->role = role;
		rolelist->priority = priority;
		rolelist->rolename = dimq_strdup(role->rolename);
