...
static void client__free_item(struct dynsec__client *client)
{
	...
	dimq_free(client->text_name);
	dimq_free(client->text_description);
	dimq_free(client->clientid);
	dimq_free(client->username);
	dimq_free(client);
}
...
int dynsec_clients__config_load(cJSON *tree)
{
	...
	cJSON_ArrayForEach(j_client, j_clients){
		if(cJSON_IsObject(j_client) == true){
			client = dimq_calloc(1, sizeof(struct dynsec__client));
			if(client == NULL){
			...
			client->username = dimq_strdup(jtmp->valuestring);
			...
			if(j_salt && cJSON_IsString(j_salt) 
				...
				if(dynsec_auth__base64_decode(j_salt->valuestring, &buf, &buf_len) != dimq_ERR_SUCCESS
				...
				if(dynsec_auth__base64_decode(j_password->valuestring, &buf, &buf_len) != dimq_ERR_SUCCESS
			...
			if(jtmp != NULL && cJSON_IsString(jtmp)){
				client->clientid = dimq_strdup(jtmp->valuestring);
			...
			if(jtmp != NULL && cJSON_IsString(jtmp)){
				client->text_name = dimq_strdup(jtmp->valuestring);
			...
			if(jtmp != NULL && cJSON_IsString(jtmp)){
				client->text_description = dimq_strdup(jtmp->valuestring);
...
static int dynsec__config_add_clients(cJSON *j_clients)
{
	...
	HASH_ITER(hh, local_clients, client, client_tmp){
		...
		if(client->pw.valid){
			if(dynsec_auth__base64_encode(client->pw.password_hash, sizeof(client->pw.password_hash), &buf) != dimq_ERR_SUCCESS){
				return 1;
			}
			...
			if(dynsec_auth__base64_encode(client->pw.salt, sizeof(client->pw.salt), &buf) != dimq_ERR_SUCCESS){
				return 1;
			}
...
int dynsec_clients__process_create(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	client->username = dimq_strdup(username);
	if(client->username == NULL){
		dynsec__command_reply(j_responses, context, "createClient", "Internal error", correlation_data);
		client__free_item(client);
		return dimq_ERR_NOMEM;
	...
	if(text_name){
		client->text_name = dimq_strdup(text_name);
		if(client->text_name == NULL){
			dynsec__command_reply(j_responses, context, "createClient", "Internal error", correlation_data);
			client__free_item(client);
			return dimq_ERR_NOMEM;
	...
	if(text_description){
		client->text_description = dimq_strdup(text_description);
		if(client->text_description == NULL){
			dynsec__command_reply(j_responses, context, "createClient", "Internal error", correlation_data);
			client__free_item(client);
			return dimq_ERR_NOMEM;
	...
	if(clientid && strlen(clientid) > 0){
		client->clientid = dimq_strdup(clientid);
		if(client->clientid == NULL){
			dynsec__command_reply(j_responses, context, "createClient", "Internal error", correlation_data);
			client__free_item(client);
			return dimq_ERR_NOMEM;
	...
	if(rc == dimq_ERR_SUCCESS || rc == ERR_LIST_NOT_FOUND){
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | createClient | username=%s | password=%s",
...
int dynsec_clients__process_delete(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username;
	struct dynsec__client *client;
	const char *admin_clientid, *admin_username;
	...
	if(client){
		...
		/* Enforce any changes */
		dimq_kick_client_by_username(username, false);

		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | deleteClient | username=%s",
...
int dynsec_clients__process_disable(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username;
	struct dynsec__client *client;
	const char *admin_clientid, *admin_username;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | disableClient | username=%s",
			admin_clientid, admin_username, username);
...
int dynsec_clients__process_enable(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username;
	struct dynsec__client *client;
	const char *admin_clientid, *admin_username;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | enableClient | username=%s",
			admin_clientid, admin_username, username);
...
int dynsec_clients__process_set_id(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | setClientId | username=%s | clientid=%s",
...
{
	if(dynsec_auth__pw_hash(client, password, client->pw.password_hash, sizeof(client->pw.password_hash), true) == dimq_ERR_SUCCESS){
		client->pw.valid = true;
...
int dynsec_clients__process_set_password(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(json_get_string(command, "password", &password, false) != dimq_ERR_SUCCESS){
	...
	if(rc == dimq_ERR_SUCCESS){
		...
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | setClientPassword | username=%s | password=******",
				admin_clientid, admin_username, username);
	}else{
...
int dynsec_clients__process_modify(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username;
	char *clientid;
	char *password;
	...
	if(json_get_string(command, "clientid", &clientid, false) == dimq_ERR_SUCCESS){
		if(clientid && strlen(clientid) > 0){
			str = dimq_strdup(clientid);
			if(str == NULL){
	...
	if(json_get_string(command, "textname", &text_name, false) == dimq_ERR_SUCCESS){
		str = dimq_strdup(text_name);
		if(str == NULL){
			dynsec__command_reply(j_responses, context, "modifyClient", "Internal error", correlation_data);
			dimq_kick_client_by_username(username, false);
			return dimq_ERR_NOMEM;
	...
	if(json_get_string(command, "textdescription", &text_description, false) == dimq_ERR_SUCCESS){
		str = dimq_strdup(text_description);
		if(str == NULL){
			dynsec__command_reply(j_responses, context, "modifyClient", "Internal error", correlation_data);
			dimq_kick_client_by_username(username, false);
			return dimq_ERR_NOMEM;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | modifyClient | username=%s",
			admin_clientid, admin_username, username);
	return dimq_ERR_SUCCESS;
}
...
int dynsec_clients__process_get(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username;
	struct dynsec__client *client;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | getClient | username=%s",
			admin_clientid, admin_username, username);
...
int dynsec_clients__process_list(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	bool verbose;
	struct dynsec__client *client, *client_tmp;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | listClients | verbose=%s | count=%d | offset=%d",
...
int dynsec_clients__process_add_role(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(json_get_string(command, "rolename", &rolename, false) != dimq_ERR_SUCCESS){
		dynsec__command_reply(j_responses, context, "addClientRole", "Invalid/missing rolename", correlation_data);
		return dimq_ERR_INVAL;
	}
	if(dimq_validate_utf8(rolename, (int)strlen(rolename)) != dimq_ERR_SUCCESS){
	...
	if(dynsec_rolelist__client_add(client, role, priority) != dimq_ERR_SUCCESS){
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | addClientRole | username=%s | rolename=%s | priority=%d",
...
int dynsec_clients__process_remove_role(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username, *rolename;
	struct dynsec__client *client;
	struct dynsec__role *role;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | removeClientRole | username=%s | rolename=%s",
