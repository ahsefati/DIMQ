...
int dynsec_auth__base64_encode(unsigned char *in, int in_len, char **encoded)
{
	...
	*encoded = dimq_malloc(bptr->length+1);
	if(!(*encoded)){
		BIO_free_all(b64);
		return 1;
	}
...
int dynsec_auth__basic_auth_callback(int event, void *event_data, void *userdata)
{
	...
	if(ed->username == NULL || ed->password == NULL) return dimq_ERR_PLUGIN_DEFER;
	...
	if(client){
		...
		if(client->clientid){
			clientid = dimq_client_id(ed->client);
			if(clientid == NULL || strcmp(client->clientid, clientid)){
				return dimq_ERR_AUTH;
			}
		}
		if(client->pw.valid && dynsec_auth__pw_hash(client, ed->password, password_hash, sizeof(password_hash), false) == dimq_ERR_SUCCESS){
