...
static void role__free_item(struct dynsec__role *role, bool remove_from_hash)
{
	...
	dimq_free(role->text_name);
	dimq_free(role->text_description);
	dimq_free(role->rolename);
...
static int add_acls_to_json(cJSON *j_role, struct dynsec__role *role)
{
	...
	if(add_single_acl_to_json(j_acls, ACL_TYPE_PUB_C_SEND, role->acls.publish_c_send) != dimq_ERR_SUCCESS
			|| add_single_acl_to_json(j_acls, ACL_TYPE_PUB_C_RECV, role->acls.publish_c_recv) != dimq_ERR_SUCCESS
			|| add_single_acl_to_json(j_acls, ACL_TYPE_SUB_LITERAL, role->acls.subscribe_literal) != dimq_ERR_SUCCESS
			|| add_single_acl_to_json(j_acls, ACL_TYPE_SUB_PATTERN, role->acls.subscribe_pattern) != dimq_ERR_SUCCESS
			|| add_single_acl_to_json(j_acls, ACL_TYPE_UNSUB_LITERAL, role->acls.unsubscribe_literal) != dimq_ERR_SUCCESS
			|| add_single_acl_to_json(j_acls, ACL_TYPE_UNSUB_PATTERN, role->acls.unsubscribe_pattern) != dimq_ERR_SUCCESS
			){
...
static int dynsec_roles__acl_load(cJSON *j_acls, const char *key, struct dynsec__acl **acllist)
{
	...
	cJSON_ArrayForEach(j_acl, j_acls){
		...
		acl = dimq_calloc(1, sizeof(struct dynsec__acl));
		if(acl == NULL){
			return 1;
		}
		...
		if(jtmp && cJSON_IsString(jtmp)){
			acl->topic = dimq_strdup(jtmp->valuestring);
		}
...
int dynsec_roles__config_load(cJSON *tree)
{
	...
	cJSON_ArrayForEach(j_role, j_roles){
		if(cJSON_IsObject(j_role) == true){
			role = dimq_calloc(1, sizeof(struct dynsec__role));
			if(role == NULL){
			...
			role->rolename = dimq_strdup(jtmp->valuestring);
			...
			if(jtmp != NULL){
				role->text_name = dimq_strdup(jtmp->valuestring);
			...
			if(jtmp != NULL){
				role->text_description = dimq_strdup(jtmp->valuestring);
...
int dynsec_roles__process_create(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	role->rolename = dimq_strdup(rolename);
	if(role->rolename == NULL){
		dynsec__command_reply(j_responses, context, "createRole", "Internal error", correlation_data);
		rc = dimq_ERR_NOMEM;
		goto error;
	...
	if(text_name){
		role->text_name = dimq_strdup(text_name);
		if(role->text_name == NULL){
			dynsec__command_reply(j_responses, context, "createRole", "Internal error", correlation_data);
			rc = dimq_ERR_NOMEM;
			goto error;
	...
	if(text_description){
		role->text_description = dimq_strdup(text_description);
		if(role->text_description == NULL){
			dynsec__command_reply(j_responses, context, "createRole", "Internal error", correlation_data);
			rc = dimq_ERR_NOMEM;
			goto error;
	...
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | createRole | rolename=%s",
...
static void role__remove_all_clients(struct dynsec__role *role)
{
	...
	HASH_ITER(hh, role->clientlist, clientlist, clientlist_tmp){
		dimq_kick_client_by_username(clientlist->client->username, false);
...
int dynsec_roles__process_delete(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(role){
		...
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | deleteRole | rolename=%s",
				admin_clientid, admin_username, rolename);
...
int dynsec_roles__process_list(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | listRoles | verbose=%s | count=%d | offset=%d",
...
int dynsec_roles__process_add_acl(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | addRoleACL | rolename=%s | acltype=%s | topic=%s | priority=%d | allow=%s",
...
int dynsec_roles__process_remove_acl(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(dimq_validate_utf8(topic, (int)strlen(topic)) != dimq_ERR_SUCCESS){
	...
	if(acl){
		...
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | removeRoleACL | rolename=%s | acltype=%s | topic=%s",
...
int dynsec_roles__process_modify(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | modifyRole | rolename=%s",
			admin_clientid, admin_username, rolename);
