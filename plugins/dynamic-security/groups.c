...
static void group__kick_all(struct dynsec__group *group)
{
	if(group == dynsec_anonymous_group){
		dimq_kick_client_by_username(NULL, false);
	}
...
static void group__free_item(struct dynsec__group *group)
{
	...
	dimq_free(group->text_name);
	dimq_free(group->text_description);
	dimq_free(group->groupname);
	dynsec_rolelist__cleanup(&group->rolelist);
	dimq_free(group);
}

int dynsec_groups__process_add_role(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(json_get_string(command, "groupname", &groupname, false) != dimq_ERR_SUCCESS){
		dynsec__command_reply(j_responses, context, "addGroupRole", "Invalid/missing groupname", correlation_data);
		return dimq_ERR_INVAL;
	}
	if(dimq_validate_utf8(groupname, (int)strlen(groupname)) != dimq_ERR_SUCCESS){
...
int dynsec_groups__config_load(cJSON *tree)
{
	...
	cJSON_ArrayForEach(j_group, j_groups){
		if(cJSON_IsObject(j_group) == true){
			group = dimq_calloc(1, sizeof(struct dynsec__group));
			if(group == NULL){
				return dimq_ERR_NOMEM;
			}

			/* Group name */
			if(json_get_string(j_group, "groupname", &str, false) != dimq_ERR_SUCCESS){
				dimq_free(group);
				continue;
			}
			...
			/* Text name */
			if(json_get_string(j_group, "textname", &str, false) == dimq_ERR_SUCCESS){
				if(str){
			...
			/* Text description */
			if(json_get_string(j_group, "textdescription", &str, false) == dimq_ERR_SUCCESS){
				if(str){
...
int dynsec_groups__process_create(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | createGroup | groupname=%s",
...
int dynsec_groups__process_delete(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *groupname;
	struct dynsec__group *group;
	const char *admin_clientid, *admin_username;
	...
	if(group){
		...
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | deleteGroup | groupname=%s",
				admin_clientid, admin_username, groupname);
...
int dynsec_groups__process_add_client(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	if(rc == dimq_ERR_SUCCESS){
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | addGroupClient | groupname=%s | username=%s | priority=%d",
...
int dynsec_groups__process_remove_client(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *username, *groupname;
	int rc;
	const char *admin_clientid, *admin_username;
	...
	if(rc == dimq_ERR_SUCCESS){
		admin_clientid = dimq_client_id(context);
		admin_username = dimq_client_username(context);
		dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | removeGroupClient | groupname=%s | username=%s",
...
int dynsec_groups__process_list(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	bool verbose;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | listGroups | verbose=%s | count=%d | offset=%d",
...
int dynsec_groups__process_get(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *groupname;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | getGroup | groupname=%s",
			admin_clientid, admin_username, groupname);
...
int dynsec_groups__process_remove_role(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *groupname, *rolename;
	struct dynsec__group *group;
	struct dynsec__role *role;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | removeGroupRole | groupname=%s | rolename=%s",
...
int dynsec_groups__process_modify(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *groupname;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | modifyGroup | groupname=%s",
			admin_clientid, admin_username, groupname);
...
int dynsec_groups__process_set_anonymous_group(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	char *groupname;
	struct dynsec__group *group = NULL;
	const char *admin_clientid, *admin_username;
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | setAnonymousGroup | groupname=%s",
			admin_clientid, admin_username, groupname);

	return dimq_ERR_SUCCESS;
}

int dynsec_groups__process_get_anonymous_group(cJSON *j_responses, struct dimq *context, cJSON *command, char *correlation_data)
{
	...
	admin_clientid = dimq_client_id(context);
	admin_username = dimq_client_username(context);
	dimq_log_printf(dimq_LOG_INFO, "dynsec: %s/%s | getAnonymousGroup",
