...
extern int tls_ex_index_dimq;

int dimq__server_certificate_verify(int preverify_ok, X509_STORE_CTX *ctx)
{
	...
	if(dimq->tls_insecure == false
#ifndef WITH_BROKER
			&& dimq->port != 0 /* no hostname checking for unix sockets */
#endif
			){
		if(X509_STORE_CTX_get_error_depth(ctx) == 0){
			...
			preverify_ok = dimq__verify_certificate_hostname(cert, dimq->bridge->addresses[dimq->bridge->cur_address].address);
#else
			preverify_ok = dimq__verify_certificate_hostname(cert, dimq->host);
#endif
			if (preverify_ok != 1) {
				log__printf(dimq, dimq_LOG_ERR, "Error: host name verification failed.");
			}
			return preverify_ok;
		}else{
			return preverify_ok;
...
static int dimq__cmp_hostname_wildcard(char *certname, const char *hostname)
{
	size_t i;
	size_t len;
 ...
 */
int dimq__verify_certificate_hostname(X509 *cert, const char *hostname)
{
	int i;
	char name[256];
	X509_NAME *subj;
...
#endif
				if(data && !dimq__cmp_hostname_wildcard((char *)data, hostname)){
	...
	if(X509_NAME_get_text_by_NID(subj, NID_commonName, name, sizeof(name)) > 0){
		...
		if (!dimq__cmp_hostname_wildcard(name, hostname)) return 1;
	}
	return 0;
}
