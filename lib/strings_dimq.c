...
const char *dimq_strerror(int dimq_errno)
{
	switch(dimq_errno){
		case dimq_ERR_AUTH_CONTINUE:
			return "Continue with authentication.";
		case dimq_ERR_NO_SUBSCRIBERS:
			return "No subscribers.";
		case dimq_ERR_SUB_EXISTS:
			return "Subscription already exists.";
		case dimq_ERR_CONN_PENDING:
		...
		case dimq_ERR_PROXY:
			return "Proxy error.";
		case dimq_ERR_MALFORMED_UTF8:
			return "Malformed UTF-8";
		case dimq_ERR_DUPLICATE_PROPERTY:
			return "Duplicate property in property list";
		case dimq_ERR_TLS_HANDSHAKE:
...
const char *dimq_connack_string(int connack_code)
{
	switch(connack_code){
		case 0:
...
const char *dimq_reason_string(int reason_code)
{
	switch(reason_code){
...
int dimq_string_to_command(const char *str, int *cmd)
{
