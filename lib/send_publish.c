...
int send__publish(struct dimq *dimq, uint16_t mid, const char *topic, uint32_t payloadlen, const void *payload, uint8_t qos, bool retain, bool dup, const dimq_property *cmsg_props, const dimq_property *store_props, uint32_t expiry_interval)
{
...
#ifdef WITH_BRIDGE
	int i;
	struct dimq__bridge_topic *cur_topic;
	bool match;
	int rc;
	...
	if(dimq->sock == INVALID_SOCKET && !dimq->wsi) return dimq_ERR_NO_CONN;
#else
...
#ifdef WITH_BROKER
	if(dimq->listener && dimq->listener->mount_point){
		len = strlen(dimq->listener->mount_point);
		if(len < strlen(topic)){
			topic += len;
		}else{
...
#ifdef WITH_BRIDGE
	if(dimq->bridge && dimq->bridge->topics && dimq->bridge->topic_remapping){
		for(i=0; i<dimq->bridge->topic_count; i++){
			cur_topic = &dimq->bridge->topics[i];
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_out)
					&& (cur_topic->remote_prefix || cur_topic->local_prefix)){
				/* Topic mapping required on this topic if the message matches */

				rc = dimq_topic_matches_sub(cur_topic->local_topic, topic, &match);
				if(rc){
					return rc;
				}
				if(match){
					mapped_topic = dimq__strdup(topic);
					if(!mapped_topic) return dimq_ERR_NOMEM;
					if(cur_topic->local_prefix){
						/* This prefix needs removing. */
						if(!strncmp(cur_topic->local_prefix, mapped_topic, strlen(cur_topic->local_prefix))){
							topic_temp = dimq__strdup(mapped_topic+strlen(cur_topic->local_prefix));
							dimq__free(mapped_topic);
							if(!topic_temp){
					...
					log__printf(NULL, dimq_LOG_DEBUG, "Sending PUBLISH to %s (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", dimq->id, dup, qos, retain, mid, mapped_topic, (long)payloadlen);
					G_PUB_BYTES_SENT_INC(payloadlen);
					rc =  send__real_publish(dimq, mid, mapped_topic, payloadlen, payload, qos, retain, dup, cmsg_props, store_props, expiry_interval);
					dimq__free(mapped_topic);
					return rc;
				}
			}
...
#endif
	log__printf(NULL, dimq_LOG_DEBUG, "Sending PUBLISH to %s (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", dimq->id, dup, qos, retain, mid, topic, (long)payloadlen);
	G_PUB_BYTES_SENT_INC(payloadlen);
#else
	log__printf(dimq, dimq_LOG_DEBUG, "Client %s sending PUBLISH (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", dimq->id, dup, qos, retain, mid, topic, (long)payloadlen);
#endif

	return send__real_publish(dimq, mid, topic, payloadlen, payload, qos, retain, dup, cmsg_props, store_props, expiry_interval);
}
...
int send__real_publish(struct dimq *dimq, uint16_t mid, const char *topic, uint32_t payloadlen, const void *payload, uint8_t qos, bool retain, bool dup, const dimq_property *cmsg_props, const dimq_property *store_props, uint32_t expiry_interval)
{
	...
	if(packet__check_oversize(dimq, packetlen)){
#ifdef WITH_BROKER
		log__printf(NULL, dimq_LOG_NOTICE, "Dropping too large outgoing PUBLISH for %s (%d bytes)", dimq->id, packetlen);
#else
		log__printf(NULL, dimq_LOG_NOTICE, "Dropping too large outgoing PUBLISH (%d bytes)", packetlen);
#endif
