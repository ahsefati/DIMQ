...
static int property__read(struct dimq__packet *packet, uint32_t *len, dimq_property *property)
{
	int rc;
	...
	if(!property) return dimq_ERR_INVAL;
	...
	*len -= 1;
	...
	memset(property, 0, sizeof(dimq_property));
	...
	switch(property_identifier){
		...
		default:
			log__printf(NULL, dimq_LOG_DEBUG, "Unsupported property type: %d", property_identifier);
...
int property__read_all(int command, struct dimq__packet *packet, dimq_property **properties)
{
	int rc;
	uint32_t proplen;
	dimq_property *p, *tail = NULL;
	...
	*properties = NULL;
	 ...
	 * same order for all */
	while(proplen > 0){
		p = dimq__calloc(1, sizeof(dimq_property));
		if(!p){
	...
	rc = dimq_property_check_all(command, *properties);
	if(rc){
		dimq_property_free_all(properties);
		return rc;
	}
...
void property__free(dimq_property **property)
{
	...
	switch((*property)->identifier){
		...
		case MQTT_PROP_SERVER_REFERENCE:
		case MQTT_PROP_REASON_STRING:
			dimq__free((*property)->value.s.v);
			break;

		case MQTT_PROP_AUTHENTICATION_DATA:
		case MQTT_PROP_CORRELATION_DATA:
			dimq__free((*property)->value.bin.v);
			break;

		case MQTT_PROP_USER_PROPERTY:
			dimq__free((*property)->name.v);
			dimq__free((*property)->value.s.v);
			break;
...
void dimq_property_free_all(dimq_property **property)
{
	dimq_property *p, *next;
...
static int property__write(struct dimq__packet *packet, const dimq_property *property)
{
	int rc;
	...
	switch(property->identifier){
		...
		default:
			log__printf(NULL, dimq_LOG_DEBUG, "Unsupported property type: %d", property->identifier);
...
int property__write_all(struct dimq__packet *packet, const dimq_property *properties, bool write_len)
{
	int rc;
	const dimq_property *p;
...
int dimq_property_check_command(int command, int identifier)
{
	switch(identifier){
		case MQTT_PROP_PAYLOAD_FORMAT_INDICATOR:
		case MQTT_PROP_MESSAGE_EXPIRY_INTERVAL:
...
const char *dimq_property_identifier_to_string(int identifier)
{
	switch(identifier){
		case MQTT_PROP_PAYLOAD_FORMAT_INDICATOR:
...
int dimq_string_to_property_info(const char *propname, int *identifier, int *type)
{
	if(!propname) return dimq_ERR_INVAL;
...
static void property__add(dimq_property **proplist, struct mqtt5__property *prop)
{
	dimq_property *p;
...
int dimq_property_add_byte(dimq_property **proplist, int identifier, uint8_t value)
{
	dimq_property *prop;

	if(!proplist) return dimq_ERR_INVAL;
	...
	prop = dimq__calloc(1, sizeof(dimq_property));
	if(!prop) return dimq_ERR_NOMEM;
...
int dimq_property_add_int16(dimq_property **proplist, int identifier, uint16_t value)
{
	dimq_property *prop;
...
int dimq_property_add_int32(dimq_property **proplist, int identifier, uint32_t value)
{
	dimq_property *prop;
...
int dimq_property_add_varint(dimq_property **proplist, int identifier, uint32_t value)
{
	...
	if(!proplist || value > 268435455) return dimq_ERR_INVAL;
	if(identifier != MQTT_PROP_SUBSCRIPTION_IDENTIFIER) return dimq_ERR_INVAL;
...
int dimq_property_add_binary(dimq_property **proplist, int identifier, const void *value, uint16_t len)
{
	dimq_property *prop;
	...
	if(len){
		prop->value.bin.v = dimq__malloc(len);
		if(!prop->value.bin.v){
			dimq__free(prop);
			return dimq_ERR_NOMEM;
		}
...
int dimq_property_add_string(dimq_property **proplist, int identifier, const char *value)
{
	dimq_property *prop;
	size_t slen = 0;
	...
	if(value){
		slen = strlen(value);
		if(dimq_validate_utf8(value, (int)slen)) return dimq_ERR_MALFORMED_UTF8;
	}
	...
	prop->identifier = identifier;
	if(value && slen > 0){
		prop->value.s.v = dimq__strdup(value);
		if(!prop->value.s.v){
			dimq__free(prop);
			return dimq_ERR_NOMEM;
		}
...
int dimq_property_add_string_pair(dimq_property **proplist, int identifier, const char *name, const char *value)
{
	dimq_property *prop;
	size_t slen_name = 0, slen_value = 0;

	if(!proplist) return dimq_ERR_INVAL;
	if(identifier != MQTT_PROP_USER_PROPERTY) return dimq_ERR_INVAL;
	if(name){
		slen_name = strlen(name);
		if(dimq_validate_utf8(name, (int)slen_name)) return dimq_ERR_MALFORMED_UTF8;
	}
	if(value){
		if(dimq_validate_utf8(value, (int)slen_value)) return dimq_ERR_MALFORMED_UTF8;
	}
...
int dimq_property_check_all(int command, const dimq_property *properties)
{
	const dimq_property *p, *tail;
	int rc;
	...
	while(p){
		...
		rc = dimq_property_check_command(command, p->identifier);
		if(rc) return rc;
		...
		if(p->identifier != MQTT_PROP_USER_PROPERTY){
			...
			while(tail){
				if(p->identifier == tail->identifier){
					return dimq_ERR_DUPLICATE_PROPERTY;
				}
				tail = tail->next;
			}
		}
...
static const dimq_property *property__get_property(const dimq_property *proplist, int identifier, bool skip_first)
{
	const dimq_property *p;
	bool is_first = true;
...
int dimq_property_identifier(const dimq_property *property)
{
...
const dimq_property *dimq_property_next(const dimq_property *proplist)
{
...
const dimq_property *dimq_property_read_byte(const dimq_property *proplist, int identifier, uint8_t *value, bool skip_first)
{
	const dimq_property *p;
...
const dimq_property *dimq_property_read_int16(const dimq_property *proplist, int identifier, uint16_t *value, bool skip_first)
{
	const dimq_property *p;
	if(!proplist) return NULL;
...
const dimq_property *dimq_property_read_int32(const dimq_property *proplist, int identifier, uint32_t *value, bool skip_first)
{
	const dimq_property *p;
	if(!proplist) return NULL;
...
const dimq_property *dimq_property_read_varint(const dimq_property *proplist, int identifier, uint32_t *value, bool skip_first)
{
	const dimq_property *p;
	if(!proplist) return NULL;
...
const dimq_property *dimq_property_read_binary(const dimq_property *proplist, int identifier, void **value, uint16_t *len, bool skip_first)
{
	const dimq_property *p;
...
const dimq_property *dimq_property_read_string(const dimq_property *proplist, int identifier, char **value, bool skip_first)
{
	const dimq_property *p;
	if(!proplist) return NULL;
...
const dimq_property *dimq_property_read_string_pair(const dimq_property *proplist, int identifier, char **name, char **value, bool skip_first)
{
	const dimq_property *p;
	if(!proplist) return NULL;
...
int dimq_property_copy_all(dimq_property **dest, const dimq_property *src)
{
	dimq_property *pnew, *plast = NULL;

	if(!src) return dimq_ERR_SUCCESS;
	if(!dest) return dimq_ERR_INVAL;

	*dest = NULL;

	while(src){
		pnew = calloc(1, sizeof(dimq_property));
		if(!pnew){
			dimq_property_free_all(dest);
			return dimq_ERR_NOMEM;
		}
		if(plast){
			plast->next = pnew;
