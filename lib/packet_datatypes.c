...
{
	assert(packet);
	if(packet->pos+1 > packet->remaining_length) return dimq_ERR_MALFORMED_PACKET;
...
{
	assert(packet);
	if(packet->pos+count > packet->remaining_length) return dimq_ERR_MALFORMED_PACKET;
...
int packet__read_binary(struct dimq__packet *packet, uint8_t **data, uint16_t *length)
{
	...
	if(packet->pos+slen > packet->remaining_length) return dimq_ERR_MALFORMED_PACKET;

	*data = dimq__malloc(slen+1U);
	if(*data){
...
int packet__read_string(struct dimq__packet *packet, char **str, uint16_t *length)
{
	...
	if(rc) return rc;
	if(*length == 0) return dimq_ERR_SUCCESS;

	if(dimq_validate_utf8(*str, *length)){
		dimq__free(*str);
		*str = NULL;
		*length = 0;
...
int packet__read_uint16(struct dimq__packet *packet, uint16_t *word)
{
	...
	assert(packet);
	if(packet->pos+2 > packet->remaining_length) return dimq_ERR_MALFORMED_PACKET;
...
{
	packet__write_byte(packet, dimq_MSB(word));
	packet__write_byte(packet, dimq_LSB(word));
}
...
int packet__read_uint32(struct dimq__packet *packet, uint32_t *word)
{
	...
	assert(packet);
	if(packet->pos+4 > packet->remaining_length) return dimq_ERR_MALFORMED_PACKET;
