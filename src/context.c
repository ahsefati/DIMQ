...
struct dimq *context__init(dimq_sock_t sock)
{
	...
	context = dimq__calloc(1, sizeof(struct dimq));
	...
	if((int)sock >= 0){
		if(!net__socket_get_address(sock, address, 1024, &context->remote_port)){
			context->address = dimq__strdup(address);
		}
		if(!context->address){
...
void context__cleanup(struct dimq *context, bool force_free)
{
	...
	dimq__free(context->auth_method);
	...
	if(context->adns){
		...
		dimq__free((struct addrinfo *)context->adns->ar_request);
...
{
	if(dimq__get_state(context) == dimq_cs_disconnected){
		return;
	}
	...
	dimq__set_state(context, dimq_cs_disconnected);
}

void context__add_to_disused(struct dimq *context)
{
	if(context->state == dimq_cs_disused) return;

	dimq__set_state(context, dimq_cs_disused);
...
{
	struct dimq *context, *next;
#ifdef WITH_WEBSOCKETS
	struct dimq *last = NULL;
#endif
