...
int config__get_dir_files(const char *include_dir, char ***files, int *file_count)
{
	...
	if(fh == INVALID_HANDLE_VALUE){
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to open include_dir '%s'.", include_dir);
		return 1;
	}
	...
	do{
		...
		l_file_count++;
		files_tmp = dimq__realloc(l_files, l_file_count*sizeof(char *));
		if(!files_tmp){
			for(i=0; i<l_file_count-1; i++){
				dimq__free(l_files[i]);
			}
			dimq__free(l_files);
			FindClose(fh);
		...
		l_files[l_file_count-1] = dimq__malloc(len+1);
	...
	while((de = readdir(dh)) != NULL){
		if(strlen(de->d_name) > 5){
			if(!strcmp(&de->d_name[strlen(de->d_name)-5], ".conf")){
				...
				l_file_count++;
				files_tmp = dimq__realloc(l_files, (size_t)l_file_count*sizeof(char *));
				if(!files_tmp){
					for(i=0; i<l_file_count-1; i++){
						dimq__free(l_files[i]);
					}
