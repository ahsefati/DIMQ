...
/* For EMFILE handling */
static dimq_sock_t spare_sock = INVALID_SOCKET;
...
{
	dimq_sock_t new_sock = INVALID_SOCKET;
	struct dimq *new_context;
#ifdef WITH_TLS
	BIO *bio;
	int rc;
	char ebuf[256];
	...
	request_init(&wrap_req, RQ_FILE, new_sock, RQ_DAEMON, "dimq", 0);
	fromhost(&wrap_req);
	if(!hosts_access(&wrap_req)){
		/* Access is denied */
		if(db.config->connection_messages == true){
			if(!net__socket_get_address(new_sock, address, 1024, NULL)){
				log__printf(NULL, dimq_LOG_NOTICE, "Client connection from %s denied access by tcpd.", address);
			}
		}
		COMPAT_CLOSE(new_sock);
		return NULL;
...
#endif
			log__printf(NULL, dimq_LOG_WARNING, "Warning: Unable to set TCP_NODELAY.");
		}
	}
	...
	if(new_context->listener->max_connections > 0 && new_context->listener->client_count > new_context->listener->max_connections){
		if(db.config->connection_messages == true){
			log__printf(NULL, dimq_LOG_NOTICE, "Client connection from %s denied: max_connections exceeded.", new_context->address);
		}
		context__cleanup(new_context, true);
		return NULL;
	}
	...
	if(db.config->connection_messages == true){
		log__printf(NULL, dimq_LOG_NOTICE, "New connection from %s:%d on port %d.",
...
static unsigned int psk_server_callback(SSL *ssl, const char *identity, unsigned char *psk, unsigned int max_psk_len)
{
	...
	psk_key = dimq__calloc(1, (size_t)max_psk_len*2 + 1);
	if(!psk_key) return 0;

	if(dimq_psk_key_get(context, psk_hint, identity, psk_key, (int)max_psk_len*2) != dimq_ERR_SUCCESS){
		dimq__free(psk_key);
		return 0;
	}

	len = dimq__hex2bin(psk_key, psk, (int)max_psk_len);
	if (len < 0){
		dimq__free(psk_key);
		return 0;
	}
	...
	if(listener->use_identity_as_username){
		context->username = dimq__strdup(identity);
		if(!context->username){
			dimq__free(psk_key);
			return 0;
		}
...
int net__tls_server_ctx(struct dimq__listener *listener)
{
	...
	if(!listener->ssl_ctx){
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to create TLS context.");
		return dimq_ERR_TLS;
	}
	...
	if(listener->tls_version == NULL){
		...
		log__printf(NULL, dimq_LOG_ERR, "Error: Unsupported tls_version \"%s\".", listener->tls_version);
		return dimq_ERR_TLS;
	}
	...
	snprintf(buf, 256, "dimq-%d", listener->port);
	...
	if(listener->ciphers){
		...
		if(rc == 0){
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to set TLS ciphers. Check cipher list \"%s\".", listener->ciphers);
			return dimq_ERR_TLS;
		}
	}else{
	...
	if(listener->ciphers_tls13){
		...
		if(rc == 0){
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to set TLS 1.3 ciphersuites. Check cipher_tls13 list \"%s\".", listener->ciphers_tls13);
			return dimq_ERR_TLS;
		}
	}
#endif
	...
	if(listener->dhparamfile){
		...
		if(!dhparamfile){
			log__printf(NULL, dimq_LOG_ERR, "Error loading dhparamfile \"%s\".", listener->dhparamfile);
			return dimq_ERR_TLS;
		}
...
#ifdef WITH_TLS
static int net__load_crl_file(struct dimq__listener *listener)
{
	X509_STORE *store;
	X509_LOOKUP *lookup;
	int rc;
	...
	if(!store){
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to obtain TLS store.");
		net__print_error(dimq_LOG_ERR, "Error: %s");
		return dimq_ERR_TLS;
	}
	lookup = X509_STORE_add_lookup(store, X509_LOOKUP_file());
	rc = X509_load_crl_file(lookup, listener->crlfile, X509_FILETYPE_PEM);
	if(rc < 1){
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load certificate revocation file \"%s\". Check crlfile.", listener->crlfile);
		net__print_error(dimq_LOG_ERR, "Error: %s");
		net__print_ssl_error(NULL);
		return dimq_ERR_TLS;
	}
...
int net__load_certificates(struct dimq__listener *listener)
{
	...
	if(rc != 1){
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load server certificate \"%s\". Check certfile.", listener->certfile);
		net__print_ssl_error(NULL);
		return dimq_ERR_TLS;
	}
	if(listener->tls_engine == NULL){
		...
		if(rc != 1){
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load server key file \"%s\". Check keyfile.", listener->keyfile);
			net__print_ssl_error(NULL);
			return dimq_ERR_TLS;
		}
	}
	...
	if(rc != 1){
		log__printf(NULL, dimq_LOG_ERR, "Error: Server certificate/key are inconsistent.");
		net__print_ssl_error(NULL);
		return dimq_ERR_TLS;
	}
...
static int net__load_engine(struct dimq__listener *listener)
{
	ENGINE *engine = NULL;
	UI_METHOD *ui_method;
	EVP_PKEY *pkey;
	...
	if(!engine){
		log__printf(NULL, dimq_LOG_ERR, "Error loading %s engine\n", listener->tls_engine);
		net__print_ssl_error(NULL);
		return dimq_ERR_TLS;
	}
	if(!ENGINE_init(engine)){
		log__printf(NULL, dimq_LOG_ERR, "Failed engine initialisation\n");
		net__print_ssl_error(NULL);
		return dimq_ERR_TLS;
	}
	ENGINE_set_default(engine, ENGINE_METHOD_ALL);

	if(listener->tls_keyform == dimq_k_engine){
		ui_method = net__get_ui_method();
		if(listener->tls_engine_kpass_sha1){
			if(!ENGINE_ctrl_cmd(engine, ENGINE_SECRET_MODE, ENGINE_SECRET_MODE_SHA, NULL, NULL, 0)){
				log__printf(NULL, dimq_LOG_ERR, "Error: Unable to set engine secret mode sha");
				net__print_ssl_error(NULL);
				return dimq_ERR_TLS;
			}
			if(!ENGINE_ctrl_cmd(engine, ENGINE_PIN, 0, listener->tls_engine_kpass_sha1, NULL, 0)){
				log__printf(NULL, dimq_LOG_ERR, "Error: Unable to set engine pin");
				net__print_ssl_error(NULL);
				return dimq_ERR_TLS;
			}
			ui_method = NULL;
		...
		if(!pkey){
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load engine private key file \"%s\".", listener->keyfile);
			net__print_ssl_error(NULL);
			return dimq_ERR_TLS;
		}
		if(SSL_CTX_use_PrivateKey(listener->ssl_ctx, pkey) <= 0){
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to use engine private key file \"%s\".", listener->keyfile);
			net__print_ssl_error(NULL);
			return dimq_ERR_TLS;
		}
	}
...
int net__tls_load_verify(struct dimq__listener *listener)
{
	...
	if(listener->cafile || listener->capath){
		...
		if(rc == 0){
			if(listener->cafile && listener->capath){
				log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load CA certificates. Check cafile \"%s\" and capath \"%s\".", listener->cafile, listener->capath);
			}else if(listener->cafile){
				log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load CA certificates. Check cafile \"%s\".", listener->cafile);
			}else{
				log__printf(NULL, dimq_LOG_ERR, "Error: Unable to load CA certificates. Check capath \"%s\".", listener->capath);
			}
		}
	}
#else
...
#ifndef WIN32
static int net__bind_interface(struct dimq__listener *listener, struct addrinfo *rp)
{
	/*
		...
		if(!strcasecmp(listener->bind_interface, ifa->ifa_name)
			...
			if(rp->ai_addr->sa_family == AF_INET){
				if(listener->host &&
					...
					log__printf(NULL, dimq_LOG_WARNING, "Warning: Interface address for %s does not match specified listener address (%s).",
	...
	freeifaddrs(ifaddr);
	log__printf(NULL, dimq_LOG_WARNING, "Warning: Interface %s does not support %s configuration.",
...
static int net__socket_listen_tcp(struct dimq__listener *listener)
{
	dimq_sock_t sock = INVALID_SOCKET;
	...
	if(!listener) return dimq_ERR_INVAL;
	...
	if (rc){
		log__printf(NULL, dimq_LOG_ERR, "Error creating listener: %s.", gai_strerror(rc));
	...
	for(rp = ainfo; rp; rp = rp->ai_next){
		if(rp->ai_family == AF_INET){
			log__printf(NULL, dimq_LOG_INFO, "Opening ipv4 listen socket on port %d.", ntohs(((struct sockaddr_in *)rp->ai_addr)->sin_port));
		}else if(rp->ai_family == AF_INET6){
			log__printf(NULL, dimq_LOG_INFO, "Opening ipv6 listen socket on port %d.", ntohs(((struct sockaddr_in6 *)rp->ai_addr)->sin6_port));
		}else{
			continue;
		}
		...
		if(sock == INVALID_SOCKET){
			net__print_error(dimq_LOG_WARNING, "Warning: %s");
			continue;
		}
		listener->sock_count++;
		listener->socks = dimq__realloc(listener->socks, sizeof(dimq_sock_t)*(size_t)listener->sock_count);
		if(!listener->socks){
		...
		if(bind(sock, rp->ai_addr, rp->ai_addrlen) == -1){
			...
			if(errno == EACCES){
				log__printf(NULL, dimq_LOG_ERR, "If you are trying to bind to a privileged port (<1024), try using setcap and do not start the broker as root:");
				log__printf(NULL, dimq_LOG_ERR, "    sudo setcap 'CAP_NET_BIND_SERVICE=+ep /usr/sbin/dimq'");
			}
#endif
			net__print_error(dimq_LOG_ERR, "Error: %s");
			COMPAT_CLOSE(sock);
...
#ifdef WITH_UNIX_SOCKETS
static int net__socket_listen_unix(struct dimq__listener *listener)
{
	...
	if(strlen(listener->unix_socket_path) > sizeof(addr.sun_path)-1){
		log__printf(NULL, dimq_LOG_ERR, "Error: Path to unix socket is too long \"%s\".", listener->unix_socket_path);
		return dimq_ERR_INVAL;
	}

	unlink(listener->unix_socket_path);
	log__printf(NULL, dimq_LOG_INFO, "Opening unix listen socket on path %s.", listener->unix_socket_path);
	...
	if(sock == INVALID_SOCKET){
		net__print_error(dimq_LOG_ERR, "Error creating unix socket: %s");
		return 1;
	}
	listener->sock_count++;
	listener->socks = dimq__realloc(listener->socks, sizeof(dimq_sock_t)*(size_t)listener->sock_count);
	...
	if(rc == -1){
		net__print_error(dimq_LOG_ERR, "Error binding unix socket: %s");
		return 1;
	}

	if(listen(sock, 10) == -1){
		net__print_error(dimq_LOG_ERR, "Error listening to unix socket: %s");
		return 1;
	}
...
#  ifdef FINAL_WITH_TLS_PSK
		if(listener->psk_hint){
			...
			if(listener->psk_hint){
				...
				if(rc == 0){
					log__printf(NULL, dimq_LOG_ERR, "Error: Unable to set TLS PSK hint.");
					net__print_ssl_error(NULL);
					return 1;
				}
			}
...
int net__socket_get_address(dimq_sock_t sock, char *buf, size_t len, uint16_t *remote_port)
{
