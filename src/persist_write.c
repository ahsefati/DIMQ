...
static int persist__client_messages_save(FILE *db_fptr, struct dimq *context, struct dimq_client_msg *queue)
{
...
static int persist__subs_save(FILE *db_fptr, struct dimq__subhier *node, const char *topic, int level)
{
	struct dimq__subhier *subhier, *subhier_tmp;
	struct dimq__subleaf *sub;
	...
	thistopic = dimq__malloc(sizeof(char)*slen);
	if(!thistopic) return dimq_ERR_NOMEM;
...
static int persist__retain_save(FILE *db_fptr, struct dimq__retainhier *node, int level)
{
	struct dimq__retainhier *retainhier, *retainhier_tmp;
...
int persist__backup(bool shutdown)
{
	int rc = 0;
	FILE *db_fptr = NULL;
	uint32_t db_version_w = htonl(dimq_DB_VERSION);
	uint32_t crc = 0;
	char *err;
	char *outfile = NULL;
	size_t len;
	...
	if(db.config == NULL) return dimq_ERR_INVAL;
	if(db.config->persistence == false) return dimq_ERR_SUCCESS;
	if(db.config->persistence_filepath == NULL) return dimq_ERR_INVAL;

	log__printf(NULL, dimq_LOG_INFO, "Saving in-memory database to %s.", db.config->persistence_filepath);

	len = strlen(db.config->persistence_filepath)+5;
	outfile = dimq__malloc(len+1);
	if(!outfile){
		log__printf(NULL, dimq_LOG_INFO, "Error saving in-memory database, out of memory.");
	...
	*
	* 24094 -rw-r--r--    2 root     root          4099 May 30 16:27 dimq.db
	* 24094 -rw-r--r--    2 root     root          4099 May 30 16:27 dimq.db.new
	*
	* The 24094 shows that dimq.db.new is hard-linked to the
	* same file as dimq.db.  If fopen(outfile, "wb") is naively
	* called then dimq.db will be truncated and the database
	* potentially corrupted.
	*
	* Any existing dimq.db.new file must be removed prior to
	...
	*/
	...
	if (rc != 0) {
		rc = 0;
		if (errno != ENOENT) {
			log__printf(NULL, dimq_LOG_INFO, "Error saving in-memory database, unable to remove %s.", outfile);
			goto error;
		}
	}
#endif

	db_fptr = dimq__fopen(outfile, "wb", true);
	if(db_fptr == NULL){
		log__printf(NULL, dimq_LOG_INFO, "Error saving in-memory database, unable to open %s for writing.", outfile);
		goto error;
	}
