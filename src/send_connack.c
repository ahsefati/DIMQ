...
int send__connack(struct dimq *context, uint8_t ack, uint8_t reason_code, const dimq_property *properties)
{
	...
	rc = dimq_property_copy_all(&connack_props, properties);
	if(rc){
		return rc;
	}

	if(context->id){
		log__printf(NULL, dimq_LOG_DEBUG, "Sending CONNACK to %s (%d, %d)", context->id, ack, reason_code);
	}else{
		log__printf(NULL, dimq_LOG_DEBUG, "Sending CONNACK to %s (%d, %d)", context->address, ack, reason_code);
	}
	...
	if(context->protocol == dimq_p_mqtt5){
		if(reason_code < 128 && db.config->retain_available == false){
			rc = dimq_property_add_byte(&connack_props, MQTT_PROP_RETAIN_AVAILABLE, 0);
			if(rc){
		...
		if(reason_code < 128 && db.config->max_packet_size > 0){
			rc = dimq_property_add_int32(&connack_props, MQTT_PROP_MAXIMUM_PACKET_SIZE, db.config->max_packet_size);
			if(rc){
				dimq_property_free_all(&connack_props);
				return rc;
			}
		...
		if(reason_code < 128 && db.config->max_inflight_messages > 0){
			rc = dimq_property_add_int16(&connack_props, MQTT_PROP_RECEIVE_MAXIMUM, db.config->max_inflight_messages);
			if(rc){
				dimq_property_free_all(&connack_props);
				return rc;
			}
		...
		if(context->listener->max_qos != 2){
			rc = dimq_property_add_byte(&connack_props, MQTT_PROP_MAXIMUM_QOS, context->listener->max_qos);
			if(rc){
				dimq_property_free_all(&connack_props);
				return rc;
			}
