...
static int bridge__create_remap_topic(const char *prefix, const char *topic, char **remap_topic)
{
	...
	if(prefix){
		if(topic){
			...
			*remap_topic = dimq__malloc(len+1);
			if(!(*remap_topic)){
			...
			*remap_topic = dimq__strdup(prefix);
			if(!(*remap_topic)){
				log__printf(NULL, dimq_LOG_ERR, "Error: Out of memory.");
				return dimq_ERR_NOMEM;
			}
		...
		*remap_topic = dimq__strdup(topic);
		if(!(*remap_topic)){
			log__printf(NULL, dimq_LOG_ERR, "Error: Out of memory.");
			return dimq_ERR_NOMEM;
		}
...
static int bridge__create_prefix(char **full_prefix, const char *topic, const char *prefix, const char *direction)
{
	...
	if(dimq_pub_topic_check(prefix) != dimq_ERR_SUCCESS){
		log__printf(NULL, dimq_LOG_ERR, "Error: Invalid bridge topic local prefix '%s'.", prefix);
	...
	*full_prefix = dimq__malloc(len);
	...
	if(dimq_sub_topic_check(*full_prefix) != dimq_ERR_SUCCESS){
...
int bridge__add_topic(struct dimq__bridge *bridge, const char *topic, enum dimq__bridge_direction direction, uint8_t qos, const char *local_prefix, const char *remote_prefix)
{
	...
	if(bridge == NULL) return dimq_ERR_INVAL;
	...
	if(local_prefix && dimq_pub_topic_check(local_prefix)){
		log__printf(NULL, dimq_LOG_ERR, "Error: Invalid bridge topic local prefix '%s'.", local_prefix);
		return dimq_ERR_INVAL;
	}
	if(remote_prefix && dimq_pub_topic_check(remote_prefix)){
		log__printf(NULL, dimq_LOG_ERR, "Error: Invalid bridge topic remote prefix '%s'.", remote_prefix);
		return dimq_ERR_INVAL;
	}
	if((topic == NULL || !strcmp(topic, "\"\"")) &&
			(local_prefix == NULL || remote_prefix == NULL)){

		log__printf(NULL, dimq_LOG_ERR, "Error: Invalid bridge remapping.");
		return dimq_ERR_INVAL;
	}


	bridge->topic_count++;
	topics = dimq__realloc(bridge->topics,
				sizeof(struct dimq__bridge_topic)*(size_t)bridge->topic_count);
	...
	if(topic == NULL || !strcmp(topic, "\"\"")){
		...
		cur_topic->topic = dimq__strdup(topic);
	...
	if(context->bridge && context->bridge->topics && context->bridge->topic_remapping){
		for(i=0; i<context->bridge->topic_count; i++){
			...
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_in)
				...
				rc = dimq_topic_matches_sub(cur_topic->remote_topic, *topic, &match);
				if(rc){
					dimq__free(*topic);
					return rc;
				}
				if(match){
					if(cur_topic->remote_prefix){
						...
						if(!strncmp(cur_topic->remote_prefix, *topic, strlen(cur_topic->remote_prefix))){
							topic_temp = dimq__strdup((*topic)+strlen(cur_topic->remote_prefix));
							if(!topic_temp){
								dimq__free(*topic);
								return dimq_ERR_NOMEM;
							}
