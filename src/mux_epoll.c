...
static void loop_handle_reads_writes(struct dimq *context, uint32_t events);

static sigset_t my_sigblock;
static struct epoll_event ep_events[MAX_EVENTS];

int mux_epoll__init(struct dimq__listener_sock *listensock, int listensock_count)
{
	...
	if ((db.epollfd = epoll_create(MAX_EVENTS)) == -1) {
		log__printf(NULL, dimq_LOG_ERR, "Error in epoll creating: %s", strerror(errno));
	...
	for(i=0; i<listensock_count; i++){
		...
		if (epoll_ctl(db.epollfd, EPOLL_CTL_ADD, listensock[i].sock, &ev) == -1) {
			log__printf(NULL, dimq_LOG_ERR, "Error in epoll initial registering: %s", strerror(errno));
...
int mux_epoll__add_out(struct dimq *context)
{
	struct epoll_event ev;
	...
	if(!(context->events & EPOLLOUT)) {
		...
		if(epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1) {
			if((errno != EEXIST)||(epoll_ctl(db.epollfd, EPOLL_CTL_MOD, context->sock, &ev) == -1)) {
				log__printf(NULL, dimq_LOG_DEBUG, "Error in epoll re-registering to EPOLLOUT: %s", strerror(errno));
			}
		}
...
int mux_epoll__remove_out(struct dimq *context)
{
	struct epoll_event ev;
	...
	if(context->events & EPOLLOUT) {
		...
		if(epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1) {
			if((errno != EEXIST)||(epoll_ctl(db.epollfd, EPOLL_CTL_MOD, context->sock, &ev) == -1)) {
					log__printf(NULL, dimq_LOG_DEBUG, "Error in epoll re-registering to EPOLLIN: %s", strerror(errno));
			}
		}
...
int mux_epoll__add_in(struct dimq *context)
{
	struct epoll_event ev;
	...
	if (epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1) {
		if(errno != EEXIST){
			log__printf(NULL, dimq_LOG_ERR, "Error in epoll accepting: %s", strerror(errno));
		}
	}
	context->events = EPOLLIN;
	return dimq_ERR_SUCCESS;
...
int mux_epoll__delete(struct dimq *context)
{
	struct epoll_event ev;
...
int mux_epoll__handle(void)
{
	...
	sigset_t origsig;
	struct dimq *context;
	struct dimq__listener_sock *listensock;
	int event_count;
	...
	case -1:
		if(errno != EINTR){
			log__printf(NULL, dimq_LOG_ERR, "Error in epoll waiting: %s.", strerror(errno));
		}
		break;
	case 0:
		break;
...
#ifdef WITH_TLS
			|| context->want_write
			|| (context->ssl && context->state == dimq_cs_new)
#endif
			){
