...
int will_delay__add(struct dimq *context)
{
	...
	item = dimq__calloc(1, sizeof(struct will_delay_list));
...
{
	if(dimq->will_delay_entry != NULL){
		DL_DELETE(delay_list, dimq->will_delay_entry);
		dimq__free(dimq->will_delay_entry);
		dimq->will_delay_entry = NULL;
	}
}
