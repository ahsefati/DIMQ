...
static void bridge__backoff_reset(struct dimq *context);
...
void bridge__start_all(void)
{
	...
	for(i=0; i<db.config->bridge_count; i++){
		if(bridge__new(&(db.config->bridges[i])) > 0){
			log__printf(NULL, dimq_LOG_WARNING, "Warning: Unable to connect to bridge %s.", 
...
{
	struct dimq *new_context = NULL;
	struct dimq **bridges;
	char *local_id;

	assert(bridge);

	local_id = dimq__strdup(bridge->local_clientid);
	...
	bridges = dimq__realloc(db.bridges, (size_t)(db.bridge_count+1)*sizeof(struct dimq *));
	if(bridges){
		db.bridges = bridges;
		db.bridge_count++;
...
int bridge__connect_step1(struct dimq *context)
{
	...
	dimq__set_state(context, dimq_cs_new);
	...
	for(i=0; i<context->bridge->topic_count; i++){
		if(context->bridge->topics[i].direction == bd_out || context->bridge->topics[i].direction == bd_both){
			log__printf(NULL, dimq_LOG_DEBUG, "Bridge %s doing local SUBSCRIBE on topic %s", context->id, context->bridge->topics[i].local_topic);
	...
	log__printf(NULL, dimq_LOG_NOTICE, "Connecting bridge (step 1) %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	...
	if(rc > 0 ){
		...
		}else if(rc == dimq_ERR_ERRNO){
			log__printf(NULL, dimq_LOG_ERR, "Error creating bridge: %s.", strerror(errno));
		}else if(rc == dimq_ERR_EAI){
			log__printf(NULL, dimq_LOG_ERR, "Error creating bridge: %s.", gai_strerror(errno));
		}
...
int bridge__connect_step2(struct dimq *context)
{
	...
	log__printf(NULL, dimq_LOG_NOTICE, "Connecting bridge (step 2) %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	...
	if(rc == dimq_ERR_CONN_PENDING){
		dimq__set_state(context, dimq_cs_connect_pending);
...
int bridge__connect_step3(struct dimq *context)
{
	...
	if(rc == dimq_ERR_SUCCESS){
		return dimq_ERR_SUCCESS;
	}else if(rc == dimq_ERR_ERRNO && errno == ENOTCONN){
		return dimq_ERR_SUCCESS;
	}else{
		if(rc == dimq_ERR_TLS){
			return rc; /* Error already printed */
...
int bridge__connect(struct dimq *context)
{
	...
	log__printf(NULL, dimq_LOG_NOTICE, "Connecting bridge %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	...
	if(rc > 0){
		...
		return rc;
	}else if(rc == dimq_ERR_CONN_PENDING){
		dimq__set_state(context, dimq_cs_connect_pending);
		mux__add_out(context);
	}
	...
	if(rc2 == dimq_ERR_SUCCESS){
		return rc;
	}else if(rc2 == dimq_ERR_ERRNO && errno == ENOTCONN){
		return dimq_ERR_SUCCESS;
	}else{
		if(rc2 == dimq_ERR_TLS){
			return rc2; /* Error already printed */
		}else if(rc2 == dimq_ERR_ERRNO){
			log__printf(NULL, dimq_LOG_ERR, "Error creating bridge: %s.", strerror(errno));
		}else if(rc2 == dimq_ERR_EAI){
			log__printf(NULL, dimq_LOG_ERR, "Error creating bridge: %s.", gai_strerror(errno));
		}
		mux__delete(context);
		net__socket_close(context);
...
int bridge__on_connect(struct dimq *context)
{
	...
	for(i=0; i<context->bridge->topic_count; i++){
		if(context->bridge->topics[i].direction == bd_in || context->bridge->topics[i].direction == bd_both){
			...
			if(context->bridge->protocol_version == dimq_p_mqtt5){
				sub_opts = sub_opts
	...
	HASH_ITER(hh_sock, db.contexts_by_sock, context, ctxt_tmp){
		if(context->bridge){
			if(mux__add_in(context)){
				log__printf(NULL, dimq_LOG_ERR, "Error in initial bridge registration: %s", strerror(errno));
...
void bridge__packet_cleanup(struct dimq *context)
{
	...
	if(context->current_out_packet){
		...
		dimq__free(context->current_out_packet);
...
static void bridge_check_pending(struct dimq *context)
{
	int err;
	socklen_t len;

	if(context->state == dimq_cs_connect_pending){
		len = sizeof(int);
		if(!getsockopt(context->sock, SOL_SOCKET, SO_ERROR, (char *)&err, &len)){
			if(err == 0){
				...
				do_disconnect(context, dimq_ERR_CONN_LOST);
				return;
			}
		}else{
			do_disconnect(context, dimq_ERR_CONN_LOST);
			...
			if(!context->bridge->restart_t){
				...
				if((context->bridge->start_type == bst_lazy && context->bridge->lazy_reconnect)
...
#else
					{
						...
						context->bridge->restart_t = 0;
						if(rc == dimq_ERR_SUCCESS || rc == dimq_ERR_CONN_PENDING){
							if(context->bridge->round_robin == false && context->bridge->cur_address != 0){
								context->bridge->primary_retry = db.now_s + 5;
							}
							mux__add_in(context);
