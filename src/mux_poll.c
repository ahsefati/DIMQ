...
int mux_poll__init(struct dimq__listener_sock *listensock, int listensock_count)
{
	size_t i;
	...
	pollfds = dimq__calloc(pollfd_max, sizeof(struct pollfd));
	if(!pollfds){
...
static int mux_poll__add(struct dimq* context, uint16_t evt)
{
	size_t i;
...
int mux_poll__add_out(struct dimq *context)
{
	return mux_poll__add(context, POLLIN | POLLOUT);
}


int mux_poll__remove_out(struct dimq *context)
{
...
int mux_poll__add_in(struct dimq *context)
{
	return mux_poll__add(context, POLLIN);
}

int mux_poll__delete(struct dimq *context)
{
	size_t pollfd_index;
...
int mux_poll__handle(struct dimq__listener_sock *listensock, int listensock_count)
{
	struct dimq *context;
	int i;
	int fdcount;
...
#  endif
		{
			log__printf(NULL, dimq_LOG_ERR, "Error in poll: %s.", strerror(errno));
		}
	}else{
