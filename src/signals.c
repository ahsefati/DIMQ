...
/*
 *
 * Signalling dimq process on Win32.
 *
 *  On Windows we we can use named events to pass signals to the dimq process.
 *  List of events :
 *
 *    dimqPID_shutdown
 *    dimqPID_reload
 *    dimqPID_backup
 *
 * (where PID is the PID of the dimq process).
 */
#ifdef WIN32
DWORD WINAPI SigThreadProc(void* data)
{
	...
	sprintf_s(evt_name, MAX_PATH, "dimq%d_shutdown", pid);
	evt[0] = CreateEvent(NULL, TRUE, FALSE, evt_name);
	sprintf_s(evt_name, MAX_PATH, "dimq%d_reload", pid);
	evt[1] = CreateEvent(NULL, FALSE, FALSE, evt_name);
	sprintf_s(evt_name, MAX_PATH, "dimq%d_backup", pid);
