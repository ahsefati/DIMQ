...
static int subs__send(struct dimq__subleaf *leaf, const char *topic, uint8_t qos, int retain, struct dimq_msg_store *stored)
{
	bool client_retain;
	uint16_t mid;
	...
	rc2 = dimq_acl_check(leaf->context, topic, stored->payloadlen, stored->payload, stored->qos, stored->retain, dimq_ACL_READ);
	if(rc2 == dimq_ERR_ACL_DENIED){
		return dimq_ERR_SUCCESS;
	}else if(rc2 == dimq_ERR_SUCCESS){
		client_qos = leaf->qos;
		...
		if(msg_qos){
			mid = dimq__mid_generate(leaf->context);
		}else{
			mid = 0;
		}
		...
		if(leaf->identifier){
			dimq_property_add_varint(&properties, MQTT_PROP_SUBSCRIPTION_IDENTIFIER, leaf->identifier);
		}
		if(db__message_insert(leaf->context, mid, dimq_md_out, msg_qos, client_retain, stored, properties, true) == 1){
			return 1;
		}
	}else{
...
static int subs__shared_process(struct dimq__subhier *hier, const char *topic, uint8_t qos, int retain, struct dimq_msg_store *stored)
{
	int rc = 0, rc2;
	struct dimq__subshared *shared, *shared_tmp;
...
static int subs__process(struct dimq__subhier *hier, const char *source_id, const char *topic, uint8_t qos, int retain, struct dimq_msg_store *stored)
{
	int rc = 0;
	int rc2;
	struct dimq__subleaf *leaf;
	...
	if(hier->subs || hier->shared){
		...
		return dimq_ERR_NO_SUBSCRIBERS;
	}
}


static int sub__add_leaf(struct dimq *context, uint8_t qos, uint32_t identifier, int options, struct dimq__subleaf **head, struct dimq__subleaf **newleaf)
{
	struct dimq__subleaf *leaf;
	...
	*newleaf = NULL;
	...
	while(leaf){
		if(leaf->context && leaf->context->id && !strcmp(leaf->context->id, context->id)){
			 ...
			 * need to update QoS. Return dimq_ERR_SUB_EXISTS to
			 * indicate this to the calling function. */
			leaf->qos = qos;
			leaf->identifier = identifier;
			return dimq_ERR_SUB_EXISTS;
		}
		leaf = leaf->next;
	}
	leaf = dimq__calloc(1, sizeof(struct dimq__subleaf));
	if(!leaf) return dimq_ERR_NOMEM;
...
static void sub__remove_shared_leaf(struct dimq__subhier *subhier, struct dimq__subshared *shared, struct dimq__subleaf *leaf)
{
	DL_DELETE(shared->subs, leaf);
	if(shared->subs == NULL){
		HASH_DELETE(hh, subhier->shared, shared);
		dimq__free(shared->name);
		dimq__free(shared);
	}
	dimq__free(leaf);
}
...
static int sub__add_shared(struct dimq *context, const char *sub, uint8_t qos, uint32_t identifier, int options, struct dimq__subhier *subhier, const char *sharename)
{
	struct dimq__subleaf *newleaf;
	struct dimq__subshared *shared = NULL;
	struct dimq__client_sub **subs;
	struct dimq__client_sub *csub;
	int i;
	size_t slen;
	int rc;
	...
	if(shared == NULL){
		shared = dimq__calloc(1, sizeof(struct dimq__subshared));
		if(!shared){
			return dimq_ERR_NOMEM;
		}
		shared->name = dimq__strdup(sharename);
		if(shared->name == NULL){
			dimq__free(shared);
			return dimq_ERR_NOMEM;
		}
	...
	if(rc != dimq_ERR_SUB_EXISTS){
		slen = strlen(sub);
		csub = dimq__calloc(1, sizeof(struct dimq__client_sub) + slen + 1);
		if(csub == NULL) return dimq_ERR_NOMEM;
		...
		if(i == context->sub_count){
			subs = dimq__realloc(context->subs, sizeof(struct dimq__client_sub *)*(size_t)(context->sub_count + 1));
			if(!subs){
	...
	if(context->protocol == dimq_p_mqtt31 || context->protocol == dimq_p_mqtt5){
		return rc;
	}else{
...
static int sub__add_normal(struct dimq *context, const char *sub, uint8_t qos, uint32_t identifier, int options, struct dimq__subhier *subhier)
{
	struct dimq__subleaf *newleaf = NULL;
	struct dimq__client_sub **subs;
	struct dimq__client_sub *csub;
	int i;
	int rc;
...
static int sub__add_context(struct dimq *context, const char *topic_filter, uint8_t qos, uint32_t identifier, int options, struct dimq__subhier *subhier, char *const *const topics, const char *sharename)
{
	struct dimq__subhier *branch;
	int topic_index = 0;
	size_t topiclen;
	...
	while(topics && topics[topic_index] != NULL){
		...
		if(!branch){
			...
			if(!branch) return dimq_ERR_NOMEM;
		}
		subhier = branch;
		topic_index++;
	}
...
static int sub__remove_normal(struct dimq *context, struct dimq__subhier *subhier, uint8_t *reason)
{
	struct dimq__subleaf *leaf;
	int i;
...
static int sub__remove_shared(struct dimq *context, struct dimq__subhier *subhier, uint8_t *reason, const char *sharename)
{
...
static int sub__remove_recurse(struct dimq *context, struct dimq__subhier *subhier, char **topics, uint8_t *reason, const char *sharename)
{
	struct dimq__subhier *branch;
...
static int sub__search(struct dimq__subhier *subhier, char **split_topics, const char *source_id, const char *topic, uint8_t qos, int retain, struct dimq_msg_store *stored)
{
	...
	if(split_topics && split_topics[0]){
		...
		if(branch){
			...
			}else if(rc != dimq_ERR_NO_SUBSCRIBERS){
				return rc;
			}
...
{
	struct dimq__subhier *child;

	assert(sibling);

	child = dimq__calloc(1, sizeof(struct dimq__subhier));
	if(!child){
	...
	child->parent = parent;
	child->topic_len = len;
	child->topic = dimq__strdup(topic);
	if(!child->topic){
		child->topic_len = 0;
		dimq__free(child);
		log__printf(NULL, dimq_LOG_ERR, "Error: Out of memory.");
...
static struct dimq__subhier *tmp_remove_subs(struct dimq__subhier *sub)
{
...
{
	int i;
	struct dimq__subhier *branch, *branch_tmp;
	struct dimq__subleaf *leaf;
