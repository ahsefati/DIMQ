...
int handle__disconnect(struct dimq *context)
{
	...
	if(context->protocol == dimq_p_mqtt5 && context->in_packet.remaining_length > 0){
	...
	log__printf(NULL, dimq_LOG_DEBUG, "Received DISCONNECT from %s", context->id);
	if(context->protocol == dimq_p_mqtt311 || context->protocol == dimq_p_mqtt5){
		if((context->in_packet.command&0x0F) != 0x00){
			do_disconnect(context, dimq_ERR_PROTOCOL);
			return dimq_ERR_PROTOCOL;
		}
	}
	if(reason_code == MQTT_RC_DISCONNECT_WITH_WILL_MSG){
		dimq__set_state(context, dimq_cs_disconnect_with_will);
	}else{
		will__clear(context);
		dimq__set_state(context, dimq_cs_disconnecting);
	}
	do_disconnect(context, dimq_ERR_SUCCESS);
