...
void __stdcall service_main(DWORD dwArgc, LPTSTR *lpszArgv)
{
	...
	service_handle = RegisterServiceCtrlHandler("dimq", service_handler);
	if(service_handle){
		memset(conf_path, 0, sizeof(conf_path));
		rc = GetEnvironmentVariable("dimq_DIR", conf_path, MAX_PATH);
		if(!rc || rc == MAX_PATH){
			service_status.dwCurrentState = SERVICE_STOPPED;
			SetServiceStatus(service_handle, &service_status);
			return;
		}
		strcat(conf_path, "/dimq.conf");

		argv = dimq__malloc(sizeof(char *)*3);
		argv[0] = "dimq";
		argv[1] = "-c";
		argv[2] = conf_path;
		argc = 3;
...
void service_install(void)
{
	...
	if(sc_manager){
		svc_handle = CreateService(sc_manager, "dimq", "dimq Broker", 
				SERVICE_START | SERVICE_STOP | SERVICE_CHANGE_CONFIG,
				SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL,
				service_string, NULL, NULL, NULL, NULL, NULL);

		if(svc_handle){
			svc_desc.lpDescription = "Eclipse dimq MQTT v5/v3.1.1 broker";
...
void service_uninstall(void)
{
	...
	if(sc_manager){
		svc_handle = OpenService(sc_manager, "dimq", SERVICE_QUERY_STATUS | DELETE);
		if(svc_handle){
