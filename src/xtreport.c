...
static void client_cost(FILE *fptr, struct dimq *context, int fn_index)
{
	long pkt_count, pkt_bytes;
	long cmsg_count;
	long cmsg_bytes;
	struct dimq__packet *pkt_tmp;
	long tBytes;
...
void xtreport(void)
{
	...
	pid = getpid();
	snprintf(filename, 40, "/tmp/xtdimq.kcg.%d.%d", pid, iter);
	iter++;
	...
	fprintf(fptr, "creator: dimq\n");
	fprintf(fptr, "pid: %d\n", pid);
	fprintf(fptr, "cmd: dimq\n\n");
