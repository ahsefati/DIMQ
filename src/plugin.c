...
static bool check_callback_exists(struct dimq__callback *cb_base, dimq_FUNC_generic_callback cb_func)
{
	struct dimq__callback *tail, *tmp;
...
static int remove_callback(struct dimq__callback **cb_base, dimq_FUNC_generic_callback cb_func)
{
	struct dimq__callback *tail, *tmp;
...
int plugin__load_v5(struct dimq__listener *listener, struct dimq__auth_plugin *plugin, struct dimq_opt *options, int option_count, void *lib)
{
	int rc;
	dimq_plugin_id_t *pid;

	if(!(plugin->plugin_init_v5 = (FUNC_plugin_init_v5)LIB_SYM(lib, "dimq_plugin_init"))){
		log__printf(NULL, dimq_LOG_ERR,
				"Error: Unable to load plugin function dimq_plugin_init().");
		LIB_ERROR();
		LIB_CLOSE(lib);
		return dimq_ERR_UNKNOWN;
	}
	if(!(plugin->plugin_cleanup_v5 = (FUNC_plugin_cleanup_v5)LIB_SYM(lib, "dimq_plugin_cleanup"))){
		log__printf(NULL, dimq_LOG_ERR,
				"Error: Unable to load plugin function dimq_plugin_cleanup().");
		LIB_ERROR();
		LIB_CLOSE(lib);
		return dimq_ERR_UNKNOWN;
	}

	pid = dimq__calloc(1, sizeof(dimq_plugin_id_t));
	if(pid == NULL){
		log__printf(NULL, dimq_LOG_ERR,
				"Error: Out of memory.");
		LIB_CLOSE(lib);
...
{
	struct dimq_evt_disconnect event_data;
	...
	DL_FOREACH(opts->plugin_callbacks.disconnect, cb_base){
		cb_base->cb(dimq_EVT_DISCONNECT, &event_data, cb_base->userdata);
	}
}
...
{
	struct dimq_evt_message event_data;
	struct dimq__callback *cb_base;
	struct dimq__security_options *opts;
	...
	DL_FOREACH(opts->plugin_callbacks.message, cb_base){
		rc = cb_base->cb(dimq_EVT_MESSAGE, &event_data, cb_base->userdata);
...
{
	struct dimq_evt_tick event_data;
	struct dimq__callback *cb_base;
	struct dimq__security_options *opts;
	...
	if(db.config->per_listener_settings){
		...
		DL_FOREACH(opts->plugin_callbacks.tick, cb_base){
			cb_base->cb(dimq_EVT_TICK, &event_data, cb_base->userdata);
		}
	}
}


int dimq_callback_register(
		...
		void *userdata)
{
	struct dimq__callback **cb_base = NULL, *cb_new;
	struct dimq__security_options *security_options;

	if(cb_func == NULL) return dimq_ERR_INVAL;
	...
	cb_new = dimq__calloc(1, sizeof(struct dimq__callback));
	if(cb_new == NULL){
		return dimq_ERR_NOMEM;
	}
...
int dimq_callback_unregister(
		dimq_plugin_id_t *identifier,
		int event,
		dimq_FUNC_generic_callback cb_func,
		const void *event_data)
{
	struct dimq__callback **cb_base = NULL;
	struct dimq__security_options *security_options;
