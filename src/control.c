...
{
	struct dimq__callback *cb_found;
	struct dimq_evt_control event_data;
	...
	if(cb_found){
		...
		rc = cb_found->cb(dimq_EVT_CONTROL, &event_data, cb_found->userdata);
		if(rc){
			if(context->protocol == dimq_p_mqtt5 && event_data.reason_string){
				dimq_property_add_string(&properties, MQTT_PROP_REASON_STRING, event_data.reason_string);
			}
		}
...
{
#ifdef WITH_CONTROL
	struct dimq__callback *cb_found, *cb_new;
	size_t topic_len;

	if(topic == NULL || cb_func == NULL) return dimq_ERR_INVAL;
	topic_len = strlen(topic);
	if(topic_len == 0 || topic_len > 65535) return dimq_ERR_INVAL;
	...
	cb_new->data = dimq__strdup(topic);
	if(cb_new->data == NULL){
		dimq__free(cb_new);
		return dimq_ERR_NOMEM;
	}
...
int control__unregister_callback(struct dimq__security_options *opts, dimq_FUNC_generic_callback cb_func, const char *topic)
{
	...
	if(topic == NULL) return dimq_ERR_INVAL;
	topic_len = strlen(topic);
	if(topic_len == 0 || topic_len > 65535) return dimq_ERR_INVAL;
	if(strncmp(topic, "$CONTROL/", strlen("$CONTROL/"))) return dimq_ERR_INVAL;

	HASH_FIND(hh, opts->plugin_callbacks.control, topic, topic_len, cb_found);
	if(cb_found && cb_found->cb == cb_func){
		HASH_DELETE(hh, opts->plugin_callbacks.control, cb_found);
		dimq__free(cb_found->data);
		dimq__free(cb_found);
