...
static void easy_address(int sock, struct dimq *dimq)
{
	char address[1024];

	if(!net__socket_get_address(sock, address, 1024, &dimq->remote_port)){
		dimq->address = dimq__strdup(address);
	}
}
		...
		size_t len)
{
	...
	switch (reason) {
		case LWS_CALLBACK_ESTABLISHED:
			dimq = context__init(WEBSOCKET_CLIENT);
			if(dimq){
				p = lws_get_protocol(wsi);
				dimq->listener = p->user;
				if(!dimq->listener){
					dimq__free(dimq);
					return -1;
				}
				...
				if(in){
					dimq->ssl = (SSL *)in;
					if(!dimq->listener->ssl_ctx){
						dimq->listener->ssl_ctx = SSL_get_SSL_CTX(dimq->ssl);
					}
				}
#endif
				u->dimq = dimq;
			...
			easy_address(lws_get_socket_fd(wsi), dimq);
			if(!dimq->address){
			...
			if(dimq->listener->max_connections > 0 && dimq->listener->client_count > dimq->listener->max_connections){
				if(db.config->connection_messages == true){
					log__printf(NULL, dimq_LOG_NOTICE, "Client connection from %s denied: max_connections exceeded.", dimq->address);
				}
			...
			dimq->sock = lws_get_socket_fd(wsi);
			HASH_ADD(hh_sock, db.contexts_by_sock, sock, sizeof(dimq->sock), dimq);
			mux__add_in(dimq);
			break;
		...
		case LWS_CALLBACK_CLOSED:
			...
			if(dimq){
				if(dimq->sock != INVALID_SOCKET){
					HASH_DELETE(hh_sock, db.contexts_by_sock, dimq);
				...
				dimq->ssl = NULL;
#endif
				do_disconnect(dimq, dimq_ERR_CONN_LOST);
			}
			break;
		...
		case LWS_CALLBACK_SERVER_WRITEABLE:
			...
			rc = db__message_write_inflight_out_latest(dimq);
			if(rc) return -1;
			rc = db__message_write_queued_out(dimq);
			if(rc) return -1;
			...
			while(dimq->current_out_packet && !lws_send_pipe_choked(dimq->wsi)){
				...
				if(count < 0){
					if (dimq->state == dimq_cs_disconnect_ws
							|| dimq->state == dimq_cs_disconnecting
							|| dimq->state == dimq_cs_disused){
		...
		case LWS_CALLBACK_RECEIVE:
			...
			while(pos < len){
				if(!dimq->in_packet.command){
					dimq->in_packet.command = buf[pos];
					pos++;
					/* Clients must send CONNECT as their first command. */
					if(dimq->state == dimq_cs_new && (dimq->in_packet.command&0xF0) != CMD_CONNECT){
						return -1;
					}
				}
				...
				if(dimq->in_packet.to_process>0){
					if((uint32_t)len - pos >= dimq->in_packet.to_process){
						memcpy(&dimq->in_packet.payload[dimq->in_packet.pos], &buf[pos], dimq->in_packet.to_process);
						dimq->in_packet.pos += dimq->in_packet.to_process;
						pos += dimq->in_packet.to_process;
						dimq->in_packet.to_process = 0;
					}else{
						memcpy(&dimq->in_packet.payload[dimq->in_packet.pos], &buf[pos], len-pos);
						dimq->in_packet.pos += (uint32_t)(len-pos);
						dimq->in_packet.to_process -= (uint32_t)(len-pos);
						return 0;
					}
				}
				...
				if(rc && (dimq->out_packet || dimq->current_out_packet)) {
		...
		size_t len)
{
	...
	switch (reason) {
		case LWS_CALLBACK_HTTP:
			...
			log__printf(NULL, dimq_LOG_DEBUG, "http serving file \"%s\".", filename_canonical);
			free(filename_canonical);
			/* FIXME - use header functions from lws 2.x */
		...
		case LWS_CALLBACK_ADD_POLL_FD:
			HASH_FIND(hh_sock, db.contexts_by_sock, &pollargs->fd, sizeof(pollargs->fd), dimq);
			if(dimq){
				if(pollargs->events & LWS_POLLOUT){
					mux__add_out(dimq);
					dimq->ws_want_write = true;
				}else{
...
static void log_wrap(int level, const char *line)
{
	...
	log__printf(NULL, dimq_LOG_WEBSOCKETS, "%s", l);
}
...
void dimq_websockets_init(struct dimq__listener *listener, const struct dimq__config *conf)
{
	...
	p = dimq__calloc(protocol_count+1, sizeof(struct lws_protocols));
	if(!p){
		log__printf(NULL, dimq_LOG_ERR, "Out of memory.");
		return;
	}
	...
	user = dimq__calloc(1, sizeof(struct libws_mqtt_hack));
	if(!user){
		dimq__free(p);
		log__printf(NULL, dimq_LOG_ERR, "Out of memory.");
		return;
	...
	if(listener->http_dir){
		...
		if(!user->http_dir){
			dimq__free(user);
			dimq__free(p);
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to open http dir \"%s\".", listener->http_dir);
			return;
		}
	}
	...
	log__printf(NULL, dimq_LOG_INFO, "Opening websockets listen socket on port %d.", listener->port);
