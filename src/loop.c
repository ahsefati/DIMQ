...
static int single_publish(struct dimq *context, struct dimq_message_v5 *msg, uint32_t message_expiry)
{
	...
	if(db__message_store(context, stored, message_expiry, 0, dimq_mo_broker)) return 1;
	...
	return db__message_insert(context, mid, dimq_md_out, (uint8_t)msg->qos, 0, stored, msg->properties, true);
}


static void read_message_expiry_interval(dimq_property **proplist, uint32_t *message_expiry)
{
	dimq_property *p, *previous = NULL;
...
{
	struct dimq_message_v5 *msg, *tmp;
	struct dimq *context;
...
{
#ifdef WITH_SYS_TREE
	time_t start_time = dimq_time();
#endif
#ifdef WITH_PERSISTENCE
	time_t last_backup = dimq_time();
#endif
#ifdef WITH_WEBSOCKETS
	int i;
#endif
...
#endif
		if(flag_reload){
			log__printf(NULL, dimq_LOG_INFO, "Reloading config.");
			config__read(db.config, true);
			listeners__reload_all_certificates();
			dimq_security_cleanup(true);
			dimq_security_init(true);
			dimq_security_apply();
...
void do_disconnect(struct dimq *context, int reason)
{
	...
	if(context->state == dimq_cs_disconnected){
		return;
	}
#ifdef WITH_WEBSOCKETS
	if(context->wsi){
		if(context->state == dimq_cs_duplicate){
			is_duplicate = true;
		}

		if(context->state != dimq_cs_disconnecting && context->state != dimq_cs_disconnect_with_will){
			dimq__set_state(context, dimq_cs_disconnect_ws);
		}
		if(context->wsi){
			...
			if(context->state != dimq_cs_disconnecting && context->state != dimq_cs_disconnect_with_will){
				switch(reason){
					case dimq_ERR_SUCCESS:
						break;
					case dimq_ERR_MALFORMED_PACKET:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to malformed packet.", id);
						break;
					case dimq_ERR_PROTOCOL:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to protocol error.", id);
						break;
					case dimq_ERR_CONN_LOST:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s closed its connection.", id);
						break;
					case dimq_ERR_AUTH:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected, not authorised.", id);
						break;
					case dimq_ERR_KEEPALIVE:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s has exceeded timeout, disconnecting.", id);
						break;
					case dimq_ERR_OVERSIZE_PACKET:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to oversize packet.", id);
						break;
					case dimq_ERR_PAYLOAD_SIZE:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to oversize payload.", id);
						break;
					case dimq_ERR_NOMEM:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to out of memory.", id);
						break;
					case dimq_ERR_NOT_SUPPORTED:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected due to using not allowed feature (QoS too high, retain not supported, or bad AUTH method).", id);
						break;
					case dimq_ERR_ADMINISTRATIVE_ACTION:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s been disconnected by administrative action.", id);
						break;
					case dimq_ERR_ERRNO:
						log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected: %s.", id, strerror(errno));
						break;
					default:
						log__printf(NULL, dimq_LOG_NOTICE, "Bad socket read/write on client %s: %s", id, dimq_strerror(reason));
						break;
				}
			}else{
				if(reason == dimq_ERR_ADMINISTRATIVE_ACTION){
					log__printf(NULL, dimq_LOG_NOTICE, "Client %s been disconnected by administrative action.", id);
				}else{
					log__printf(NULL, dimq_LOG_NOTICE, "Client %s disconnected.", id);
				}
			}
		}
		mux__delete(context);
