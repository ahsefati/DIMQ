...
static unsigned int log_priorities = dimq_LOG_ERR | dimq_LOG_WARNING | dimq_LOG_NOTICE | dimq_LOG_INFO;
...
void dlt_fifo_check(void)
{
	 ...
	 * people using DLT create the fifo after dimq has started, but at the
...
#ifndef WIN32
		openlog("dimq", LOG_PID|LOG_CONS, config->log_facility);
#else
		syslog_h = OpenEventLog(NULL, "dimq");
#endif
	}

	if(log_destinations & MQTT3_LOG_FILE){
		config->log_fptr = dimq__fopen(config->log_file, "at", true);
		if(config->log_fptr){
			setvbuf(config->log_fptr, log_fptr_buffer, _IOLBF, sizeof(log_fptr_buffer));
		}else{
			log_destinations = MQTT3_LOG_STDERR;
			log_priorities = dimq_LOG_ERR;
			log__printf(NULL, dimq_LOG_ERR, "Error: Unable to open log file %s for writing.", config->log_file);
		}
	}
#ifdef WITH_DLT
	dlt_fifo_check();
	if(dlt_allowed){
		DLT_REGISTER_APP("MQTT","dimq log");
		dlt_register_context(&dltContext, "MQTT", "dimq DLT context");
	}
#endif
	return rc;
}
...
DltLogLevelType get_dlt_level(unsigned int priority)
{
	switch (priority) {
		...
		case dimq_LOG_WARNING:
			return DLT_LOG_WARN;
		case dimq_LOG_INFO:
			return DLT_LOG_INFO;
		case dimq_LOG_DEBUG:
			return DLT_LOG_DEBUG;
		case dimq_LOG_NOTICE:
		case dimq_LOG_SUBSCRIBE:
		case dimq_LOG_UNSUBSCRIBE:
...
static int log__vprintf(unsigned int priority, const char *fmt, va_list va)
{
	...
	if((log_priorities & priority) && log_destinations != MQTT3_LOG_NONE){
		...
		if(log_destinations & MQTT3_LOG_TOPIC && priority != dimq_LOG_DEBUG && priority != dimq_LOG_INTERNAL){
			db__messages_easy_queue(NULL, topic, 2, (uint32_t)strlen(log_line), log_line, 0, 20, NULL);
		}
#ifdef WITH_DLT
		if(log_destinations & MQTT3_LOG_DLT && priority != dimq_LOG_INTERNAL){
...
void log__internal(const char *fmt, ...)
{
	...
	if(len >= 200){
		log__printf(NULL, dimq_LOG_INTERNAL, "Internal log buffer too short (%d)", len);
		return;
	}

#ifdef WIN32
	log__printf(NULL, dimq_LOG_INTERNAL, "%s", buf);
#else
	log__printf(NULL, dimq_LOG_INTERNAL, "%s%s%s", "\e[32m", buf, "\e[0m");
#endif
}

int dimq_log_vprintf(int level, const char *fmt, va_list va)
{
