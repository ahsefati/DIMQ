...
static struct dimq *persist__find_or_add_context(const char *client_id, uint16_t last_mid)
{
	struct dimq *context;
	...
	if(!context){
		...
		context->id = dimq__strdup(client_id);
		if(!context->id){
			dimq__free(context);
			return NULL;
		}
...
{
	struct dimq_client_msg *cmsg;
	struct dimq_msg_store_load *load;
	struct dimq *context;
	...
	if(!context){
		log__printf(NULL, dimq_LOG_WARNING, "Warning: Persistence file contains client message with no matching client. File may be corrupt.");
		return 0;
	}

	cmsg = dimq__calloc(1, sizeof(struct dimq_client_msg));
	if(!cmsg){
		log__printf(NULL, dimq_LOG_ERR, "Error: Out of memory.");
		return dimq_ERR_NOMEM;
	}
	...
	if(cmsg->direction == dimq_md_out){
		msg_data = &context->msgs_out;
	}else{
		msg_data = &context->msgs_in;
	}

	if(chunk->F.state == dimq_ms_queued || (chunk->F.qos > 0 && msg_data->inflight_quota == 0)){
...
static int persist__client_chunk_restore(FILE *db_fptr)
{
	...
	if(rc > 0){
		...
		log__printf(NULL, dimq_LOG_WARNING, "Warning: Empty client entry found in persistence database, it may be corrupt.");
		return dimq_ERR_SUCCESS;
	}
	...
	dimq__free(chunk.client_id);
	if(chunk.username){
		dimq__free(chunk.username);
	}
	return rc;
}
...
static int persist__msg_store_chunk_restore(FILE *db_fptr, uint32_t length)
{
	...
	struct dimq_msg_store *stored = NULL;
	struct dimq_msg_store_load *load;
	...
	load = dimq__calloc(1, sizeof(struct dimq_msg_store_load));
	if(!load){
		dimq__free(chunk.source.id);
		dimq__free(chunk.source.username);
		dimq__free(chunk.topic);
		dimq__free(chunk.payload);
		log__printf(NULL, dimq_LOG_ERR, "Error: Out of memory.");
		return dimq_ERR_NOMEM;
	}
	...
	rc = db__message_store(&chunk.source, stored, message_expiry_interval,
			chunk.F.store_id, dimq_mo_client);
...
int persist__restore(void)
{
	...
	size_t rlen;
	char *err;
	struct dimq_msg_store_load *load, *load_tmp;
	...
	fptr = dimq__fopen(db.config->persistence_filepath, "rb", false);
	if(fptr == NULL) return dimq_ERR_SUCCESS;
	rlen = fread(&header, 1, 15, fptr);
	if(rlen == 0){
		fclose(fptr);
		log__printf(NULL, dimq_LOG_WARNING, "Warning: Persistence file is empty.");
		return 0;
	}else if(rlen != 15){
		goto error;
	}
	if(!memcmp(header, magic, 15)){
		...
		if(db_version != dimq_DB_VERSION){
			if(db_version == 5){
				...
				fclose(fptr);
				log__printf(NULL, dimq_LOG_ERR, "Error: Unsupported persistent database format version %d (need version %d).", db_version, dimq_DB_VERSION);
				return 1;
			}
		}

		while(persist__chunk_header_read(fptr, &chunk, &length) == dimq_ERR_SUCCESS){
			switch(chunk){
				case DB_CHUNK_CFG:
					if(db_version == 6 || db_version == 5){
					...
					if(cfg_chunk.dbid_size != sizeof(dbid_t)){
						log__printf(NULL, dimq_LOG_ERR, "Error: Incompatible database configuration (dbid size is %d bytes, expected %lu)",
				...
				default:
					log__printf(NULL, dimq_LOG_WARNING, "Warning: Unsupported chunk \"%d\" in persistent database file. Ignoring.", chunk);
					fseek(fptr, length, SEEK_CUR);
					break;
			}
		}
	}else{
		log__printf(NULL, dimq_LOG_ERR, "Error: Unable to restore persistent database. Unrecognised file format.");
		rc = 1;
	}
...
error:
	err = strerror(errno);
	log__printf(NULL, dimq_LOG_ERR, "Error: %s.", err);
	if(fptr) fclose(fptr);
	return 1;
}
