...
{
	/* Empty packet */
	byte_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, 0);
}
...
static void TEST_byte_read_success(void)
{
	...
	payload[0] = 0x00;
	byte_read_helper(payload, 1, dimq_ERR_SUCCESS, 0x00);

	/* Middle */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x1F;
	byte_read_helper(payload, 1, dimq_ERR_SUCCESS, 0x1F);

	/* 255 value */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0xFF;
	byte_read_helper(payload, 1, dimq_ERR_SUCCESS, 0xFF);
...
{
	/* Empty packet */
	uint16_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, 0);
}
...
static void TEST_uint16_read_truncated(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x38;
	uint16_read_helper(payload, 1, dimq_ERR_MALFORMED_PACKET, 0);
}
...
static void TEST_uint16_read_success(void)
{
	...
	payload[0] = 0x00;
	payload[1] = 0x00;
	uint16_read_helper(payload, 2, dimq_ERR_SUCCESS, 0x0000);

	/* Endian check */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x38;
	payload[1] = 0xF3;
	uint16_read_helper(payload, 2, dimq_ERR_SUCCESS, 0x38F3);

	/* 65,535 value */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0xFF;
	payload[1] = 0xFF;
	uint16_read_helper(payload, 2, dimq_ERR_SUCCESS, 0xFFFF);
...
{
	/* Empty packet */
	uint32_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, 0);
}
...
static void TEST_uint32_read_truncated(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x38;
	uint32_read_helper(payload, 1, dimq_ERR_MALFORMED_PACKET, 0);

	/* 2 byte packet */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x38;
	payload[1] = 0x38;
	uint32_read_helper(payload, 2, dimq_ERR_MALFORMED_PACKET, 0);
	...
	payload[1] = 0x38;
	payload[2] = 0x38;
	uint32_read_helper(payload, 3, dimq_ERR_MALFORMED_PACKET, 0);
}
...
static void TEST_uint32_read_success(void)
{
	...
	payload[2] = 0x00;
	payload[3] = 0x00;
	uint32_read_helper(payload, 4, dimq_ERR_SUCCESS, 0x00000000);
	...
	payload[2] = 0x56;
	payload[3] = 0x78;
	uint32_read_helper(payload, 4, dimq_ERR_SUCCESS, 0x12345678);
	...
	payload[2] = 0xFF;
	payload[3] = 0xFF;
	uint32_read_helper(payload, 4, dimq_ERR_SUCCESS, 0xFFFFFFFF);
...
{
	/* Empty packet */
	varint_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);
}
...
static void TEST_varint_read_truncated(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x80;
	varint_read_helper(payload, 1, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);

	/* Varint bigger than packet */
	memset(payload, 1, sizeof(payload));
	payload[0] = 0x80;
	payload[1] = 0x80;
	varint_read_helper(payload, 2, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);
	...
	payload[1] = 0x80;
	payload[2] = 0x80;
	varint_read_helper(payload, 3, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);
	...
	payload[2] = 0x80;
	payload[3] = 0x80;
	varint_read_helper(payload, 4, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);
}
...
static void TEST_varint_read_boundaries(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x00;
	varint_read_helper(payload, 1, dimq_ERR_SUCCESS, 0, 1);

	/* Value = 127 (just beneath the crossover to two bytes */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x7F;
	varint_read_helper(payload, 1, dimq_ERR_SUCCESS, 127, 1);

	/* Value = 128 (just after the crossover to two bytes */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x80;
	payload[1] = 0x01;
	varint_read_helper(payload, 2, dimq_ERR_SUCCESS, 128, 2);

	/* Value = 16383 (just before the crossover to three bytes */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0xFF;
	payload[1] = 0x7F;
	varint_read_helper(payload, 2, dimq_ERR_SUCCESS, 16383, 2);
	...
	payload[1] = 0x80;
	payload[2] = 0x01;
	varint_read_helper(payload, 3, dimq_ERR_SUCCESS, 16384, 3);
	...
	payload[1] = 0xFF;
	payload[2] = 0x7F;
	varint_read_helper(payload, 3, dimq_ERR_SUCCESS, 2097151, 3);
	...
	payload[2] = 0x80;
	payload[3] = 0x01;
	varint_read_helper(payload, 4, dimq_ERR_SUCCESS, 2097152, 4);
	...
	payload[2] = 0xFF;
	payload[3] = 0x7F;
	varint_read_helper(payload, 4, dimq_ERR_SUCCESS, 268435455, 4);
}
...
static void TEST_varint_read_5_bytes(void)
{
	...
	payload[3] = 0x80;
	payload[4] = 0x01;
	varint_read_helper(payload, 5, dimq_ERR_MALFORMED_PACKET, UINT32_MAX, UINT8_MAX);
}
...
{
	string_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
}
...
static void TEST_string_read_truncated(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x02;
	string_read_helper(payload, 1, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);

	/* 2 byte packet */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x02;
	payload[1] = 0x02;
	string_read_helper(payload, 2, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
	...
	payload[1] = 0x02;
	payload[2] = 'a';
	string_read_helper(payload, 3, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
	...
	payload[2] = 'a';
	payload[3] = 'b';
	string_read_helper(payload, 4, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
}
...
static void TEST_string_read_empty_string(void)
{
	...
	payload[0] = 0x00;
	payload[1] = 0x00;
	string_read_helper(payload, 2, dimq_ERR_SUCCESS, (const uint8_t *)"", 0);
}
...
static void TEST_string_read_valid_string(void)
{
	...
	payload[11] = 'n';
	payload[12] = 'g';
	string_read_helper(payload, 13, dimq_ERR_SUCCESS, (const uint8_t *)"test string", 11);
}
...
static void TEST_string_read_malformed_string(void)
{
	...
	payload[7] = 0xA0;
	payload[8] = 0x80;
	string_read_helper(payload, 9, dimq_ERR_MALFORMED_UTF8, NULL, 0);
}
...
static void TEST_string_read_mqtt_1_5_4_3(void)
{
	...
	payload[11] = 's';
	payload[12] = 't';
	string_read_helper(payload, 13, dimq_ERR_SUCCESS, &payload[2], 11);
}
...
{
	binary_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
}
...
static void TEST_binary_data_read_truncated(void)
{
	...
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x02;
	binary_read_helper(payload, 1, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);

	/* 2 byte packet */
	memset(payload, 0, sizeof(payload));
	payload[0] = 0x02;
	payload[1] = 0x02;
	binary_read_helper(payload, 2, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
	...
	payload[1] = 0x02;
	payload[2] = 'a';
	binary_read_helper(payload, 3, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
	...
	payload[2] = 'a';
	payload[3] = 'b';
	binary_read_helper(payload, 4, dimq_ERR_MALFORMED_PACKET, NULL, UINT16_MAX);
}
...
{
	/* Empty packet */
	bytes_read_helper(NULL, 0, dimq_ERR_SUCCESS, NULL, 0);
}
...
{
	bytes_read_helper(NULL, 0, dimq_ERR_MALFORMED_PACKET, NULL, 1);
}
...
static void TEST_bytes_read_success(void)
{
	...
	bytes_read_helper(payload, 20, dimq_ERR_SUCCESS, payload, 20);
}
