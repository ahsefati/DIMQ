...
static void hier_quick_check(struct dimq__subhier **sub, struct dimq *context, const char *topic)
{
	if(sub != NULL){
...
static void TEST_sub_add_single(void)
{
	...
	struct dimq context;
	struct dimq__subhier *sub;
	int rc;

	memset(&db, 0, sizeof(struct dimq_db));
	memset(&config, 0, sizeof(struct dimq__config));
	memset(&listener, 0, sizeof(struct dimq__listener));
	memset(&context, 0, sizeof(struct dimq));
