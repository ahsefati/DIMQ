...
def do_test(will_props, recvd_props):
    ...
    mid = 1
    connect1_packet = dimq_test.gen_connect("will-helper", keepalive=keepalive, proto_ver=5)
    connack1_packet = dimq_test.gen_connack(rc=0, proto_ver=5)

    subscribe1_packet = dimq_test.gen_subscribe(mid, "will/test", 0, proto_ver=5)
    suback1_packet = dimq_test.gen_suback(mid, 0, proto_ver=5)

    connect2_packet = dimq_test.gen_connect("will-test", keepalive=keepalive, proto_ver=5, will_topic="will/test", will_payload=b"will payload", will_properties=will_props)
    connack2_packet = dimq_test.gen_connack(rc=0, proto_ver=5)

    publish_packet = dimq_test.gen_publish("will/test", qos=0, payload="will payload", proto_ver=5, properties=recvd_props)
