...
def do_test(proto_ver, cs, lcs=None):
    ...
    (port_a_listen, port_b_listen) = dimq_test.get_port(2)
    ...
    def make_conn(client_tag, proto, cs, session_present=False):
        ...
        keepalive = 60
        conn = dimq_test.gen_connect(client_id, keepalive=keepalive, clean_session=cs, proto_ver=proto, session_expiry=0 if cs else 5000)
        connack = dimq_test.gen_connack(rc=0, proto_ver=proto_ver, flags=1 if session_present else 0)
    ...
    def make_sub(topic, mid, qos, proto):
        if proto_ver == 5:
            ...
            opts = 0
        sub = dimq_test.gen_subscribe(mid, topic, qos | opts, proto_ver=proto)
        suback = dimq_test.gen_suback(mid, qos, proto_ver=proto)
    ...
    def make_pub(topic, mid, proto, qos=1, payload_tag="message", rc=-1):
        ...
        pub = dimq_test.gen_publish(topic, mid=mid, qos=qos, retain=False, payload=payload_tag + "-from-" + topic, proto_ver=proto)
        puback = dimq_test.gen_puback(mid, proto_ver=proto, reason_code=rc)
    ...
    try:
        ...
        broker_b = dimq_test.start_broker(filename=conf_file_b, port=port_b_listen, use_conf=True)
        broker_a = dimq_test.start_broker(filename=conf_file_a, port=port_a_listen, use_conf=True)

        client_a = dimq_test.do_client_connect(conn_a.p, conn_a.ack, port=port_a_listen)
        dimq_test.do_send_receive(client_a, sub_a.p, sub_a.ack, "suback_a")

        client_b = dimq_test.do_client_connect(conn_b.p, conn_b.ack, port=port_b_listen)
        dimq_test.do_send_receive(client_b, sub_b.p, sub_b.ack, "suback_b")

        dimq_test.do_send_receive(client_a, pub_a1.p, pub_a1.ack, "puback_a1")
        dimq_test.do_receive_send(client_b, pub_a1.p, pub_a1.ack, "a->b1 (b-side)")

        dimq_test.do_send_receive(client_b, pub_b1.p, pub_b1.ack, "puback_b1")
        dimq_test.do_receive_send(client_a, pub_b1.p, pub_b1.ack, "b->a1 (a-side)")
        ...
        dimq_test.do_send_receive(client_a, pub_a2.p, pub_a2.ack, "puback_a2")

        broker_b = dimq_test.start_broker(filename=conf_file_b, port=port_b_listen, use_conf=True)
        # client b needs to reconnect now!

        client_b = dimq_test.do_client_connect(reconn_b.p, reconn_b.ack, port=port_b_listen)
        ...
        dimq_test.do_send_receive(client_a, pub_a3.p, pub_a3.ack, "puback_a3")


        if expect_queued_ab:
            tprint("1.expecting a->b queueing")
            dimq_test.do_receive_send(client_b, pub_a2.p, pub_a2.ack, "a->b_2")
            dimq_test.do_receive_send(client_b, pub_a3.p, pub_a3.ack, "a->b_3")
        else:
            tprint("not expecting a->b queueing")
            dimq_test.do_receive_send(client_b, pub_a3r.p, pub_a3r.ack, "a->b_3(r)")
        ...
        dimq_test.do_send_receive(client_b, pub_b2.p, pub_b2.ack, "puback_b2")

        broker_a = dimq_test.start_broker(filename=conf_file_a, port=port_a_listen, use_conf=True)
        # client a needs to reconnect now!
        client_a = dimq_test.do_client_connect(reconn_a.p, reconn_a.ack, port=port_a_listen)
        tprint("client A reconnected after restarting broker A at ", time.time())
        # Need to sleep long enough to be sure of a re-connection...
        time.sleep(10)  # yuck, this makes the test run for ages!

        # should go through
        dimq_test.do_send_receive(client_b, pub_b3.p, pub_b3.ack, "puback_b3")

        if expect_queued_ba:
            tprint("2.expecting b->a queueueing")
            dimq_test.do_receive_send(client_a, pub_b2.p, pub_b2.ack, "b->a_2")
            dimq_test.do_receive_send(client_a, pub_b3.p, pub_b3.ack, "b->a_3")
        else:
            tprint("not expecting message b->a_2")
            dimq_test.do_receive_send(client_a, pub_b3r.p, pub_b3r.ack, "b->a_3(r)")
...
# https://github.com/eclipse/dimq/issues/1632
