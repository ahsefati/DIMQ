...
connect1_packet = dimq_test.gen_connect("sub", proto_ver=5, keepalive=keepalive)
...
connect2_packet = dimq_test.gen_connect("connect-disconnect-test", proto_ver=5, keepalive=keepalive, will_topic="failure", will_payload=b"failure")
...
try:
    ...
    disconnect_packet = dimq_test.gen_disconnect(proto_ver=5, reason_code=0)
    disco_test("disco len=1", disconnect_packet)

    # Reason code, empty properties, len=2
    disconnect_packet = dimq_test.gen_disconnect(proto_ver=5, reason_code=0, properties="")
    ...
    disconnect_packet = dimq_test.gen_disconnect(proto_ver=5, reason_code=0, properties=props)
    disco_test("disco len>2", disconnect_packet)
