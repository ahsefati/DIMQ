...
def helper(port, pub_topic):
    ...
    publish_packet = dimq_test.gen_publish(pub_topic, qos=0, retain=True, payload="message")
...
def pattern_test(sub_topic, pub_topic):
    rc = 1
    keepalive = 60
    connect_packet = dimq_test.gen_connect("pattern-sub-test", keepalive=keepalive)
    connack_packet = dimq_test.gen_connack(rc=0)

    publish_packet = dimq_test.gen_publish(pub_topic, qos=0, payload="message")
    publish_retained_packet = dimq_test.gen_publish(pub_topic, qos=0, retain=True, payload="message")

    mid = 312
    subscribe_packet = dimq_test.gen_subscribe(mid, sub_topic, 0)
    suback_packet = dimq_test.gen_suback(mid, 0)

    mid = 234;
    unsubscribe_packet = dimq_test.gen_unsubscribe(mid, sub_topic)
    ...
    try:
        ...
        dimq_test.do_send_receive(sock, subscribe_packet, suback_packet, "suback")
        dimq_test.expect_packet(sock, "publish retained", publish_retained_packet)
        rc = 0
