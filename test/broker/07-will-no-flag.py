...
def do_test(proto_ver):
    rc = 1
    keepalive = 10
    connect_packet = dimq_test.gen_connect("will-no-payload", keepalive=keepalive, will_topic="will/topic", will_qos=1, will_retain=True, proto_ver=proto_ver)
    ...
    try:
        sock = dimq_test.do_client_connect(connect_packet, b"", port=port)
        sock.close()
        rc = 0
