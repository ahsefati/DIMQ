...
def do_test(proto_ver, outgoing_retain):
    ...
    if outgoing_retain == "true":
        publish_packet = dimq_test.gen_publish("bridge/retain/test", qos=0, retain=True, payload="message", proto_ver=proto_ver)
    else:
        publish_packet = dimq_test.gen_publish("bridge/retain/test", qos=0, retain=False, payload="message", proto_ver=proto_ver)


    helper_connect_packet = dimq_test.gen_connect("helper", keepalive=keepalive, clean_session=True, proto_ver=proto_ver)
    helper_connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)
    helper_publish_packet = dimq_test.gen_publish("bridge/retain/test", qos=0, retain=True, payload="message", proto_ver=proto_ver)
    ...
    try:
        ...
        helper = dimq_test.do_client_connect(helper_connect_packet, helper_connack_packet, port=port2)
