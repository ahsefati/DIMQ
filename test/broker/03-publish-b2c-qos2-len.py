...
def helper(port):
    ...
    mid = 1
    publish_packet = dimq_test.gen_publish("qos2/len/test", qos=2, mid=mid, payload="len-message")
...
def len_test(test, pubrec_packet, pubcomp_packet):
    ...
    mid = 3265
    keepalive = 60
    connect_packet = dimq_test.gen_connect("pub-test", keepalive=keepalive, clean_session=False, proto_ver=5)
    connack_packet = dimq_test.gen_connack(flags=0, rc=0, proto_ver=5)

    subscribe_packet = dimq_test.gen_subscribe(mid, "qos2/len/test", 2, proto_ver=5)
    suback_packet = dimq_test.gen_suback(mid, 2, proto_ver=5)

    mid = 1
    publish_packet = dimq_test.gen_publish("qos2/len/test", qos=2, mid=mid, payload="len-message", proto_ver=5)
    pubrel_packet = dimq_test.gen_pubrel(mid)
...
pubrec_packet = dimq_test.gen_pubrec(1)
pubcomp_packet = dimq_test.gen_pubcomp(1)
len_test("qos2 len 2", pubrec_packet, pubcomp_packet)

# Reason code, no properties
pubrec_packet = dimq_test.gen_pubrec(1, proto_ver=5, reason_code=0x00)
pubcomp_packet = dimq_test.gen_pubcomp(1, proto_ver=5, reason_code=0x00)
len_test("qos2 len 3", pubrec_packet, pubcomp_packet)

# Reason code, empty properties
pubrec_packet = dimq_test.gen_pubrec(1, proto_ver=5, reason_code=0x00, properties="")
pubcomp_packet = dimq_test.gen_pubcomp(1, proto_ver=5, reason_code=0x00, properties="")
...
pubrec_packet = dimq_test.gen_pubrec(1, proto_ver=5, reason_code=0x00, properties=props)
props = mqtt5_props.gen_string_pair_prop(mqtt5_props.PROP_USER_PROPERTY, "key", "value")
pubcomp_packet = dimq_test.gen_pubcomp(1, proto_ver=5, reason_code=0x00, properties=props)
