...
def do_test(publish_packet, reason_code, error_string):
    ...
    connack_packet = dimq_test.gen_connack(rc=0, proto_ver=5, properties=connack_props, property_helper=False)
    ...
    dimq_test.do_send_receive(sock, publish_packet, disconnect_packet, error_string=error_string)
    rc = 0
...
try:
    # mid == 0
    publish_packet = dimq_test.gen_publish(topic="test/topic", qos=1, mid=0, proto_ver=5)
    do_test(publish_packet, mqtt5_rc.MQTT_RC_PROTOCOL_ERROR, "mid == 0")

    # qos > 2
    publish_packet = dimq_test.gen_publish(topic="test/topic", qos=3, mid=1, proto_ver=5)
    do_test(publish_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "qos > 2")

    # qos > maximum qos
    publish_packet = dimq_test.gen_publish(topic="test/topic", qos=2, mid=1, proto_ver=5)
    do_test(publish_packet, mqtt5_rc.MQTT_RC_QOS_NOT_SUPPORTED, "qos > maximum qos")

    # retain not supported
    publish_packet = dimq_test.gen_publish(topic="test/topic", qos=0, retain=True, proto_ver=5, payload="a")
    ...
    publish_packet = dimq_test.gen_publish(topic="test/topic", qos=1, mid=1, proto_ver=5, properties=props)
    ...
    # Bad topic
    publish_packet = dimq_test.gen_publish(topic="#/test/topic", qos=1, mid=1, proto_ver=5)
