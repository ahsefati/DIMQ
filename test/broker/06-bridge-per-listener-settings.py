...
def write_config(filename, port1, port2, port3, protocol_version):
    with open(filename, 'w') as f:
        ...
        f.write("topic clients/total in 0 test/dimq/org $SYS/broker/\n")
...
def inner_test(bridge, sock, proto_ver):
    ...
    if not dimq_test.expect_packet(bridge, "connect", connect_packet):
        return 1
    ...
    for pattern in ("remote/topic/#", "remote2/topic/prefix/#", "remote3/topic/+/value"):
        mid += 1
        subscribe_packet = dimq_test.gen_subscribe(mid, pattern, 0 | opts, proto_ver=proto_ver)
        suback_packet = dimq_test.gen_suback(mid, 0, proto_ver=proto_ver)
        if not dimq_test.expect_packet(bridge, "subscribe", subscribe_packet):
            return 1
        bridge.send(suback_packet)

    mid += 1
    subscribe_packet = dimq_test.gen_subscribe(mid, "#", 0 | opts, proto_ver=proto_ver)
    suback_packet = dimq_test.gen_suback(mid, 0, proto_ver=proto_ver)
    sock.send(subscribe_packet)
    if not dimq_test.expect_packet(sock, "suback", suback_packet):
        return 1
    ...
    cases = [
        ...
        ('test/dimq/orgclients/total', '$SYS/broker/clients/total'),
    ]

    for (local_topic, remote_topic) in cases:
        mid += 1
        remote_publish_packet = dimq_test.gen_publish(
            remote_topic, qos=0, mid=mid, payload='', proto_ver=proto_ver)
        local_publish_packet = dimq_test.gen_publish(
            local_topic, qos=0, mid=mid, payload='', proto_ver=proto_ver)

        bridge.send(remote_publish_packet)
        match = dimq_test.expect_packet(sock, "publish", local_publish_packet)
        if not match:
...
def do_test(proto_ver):
    ...
    (port1, port2, port3) = dimq_test.get_port(3)
    ...
    client_connect_packet = dimq_test.gen_connect("pub-test", keepalive=keepalive, proto_ver=proto_ver)
    client_connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)
    ...
    try:
        ...
        sock = dimq_test.do_client_connect(
