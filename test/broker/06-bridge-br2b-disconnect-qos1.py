...
def do_test(proto_ver):
    ...
    mid = 1
    subscribe_packet = dimq_test.gen_subscribe(mid, "bridge/#", 1 | opts, proto_ver=proto_ver)
    suback_packet = dimq_test.gen_suback(mid, 1, proto_ver=proto_ver)

    mid = 3
    subscribe2_packet = dimq_test.gen_subscribe(mid, "bridge/#", 1 | opts, proto_ver=proto_ver)
    suback2_packet = dimq_test.gen_suback(mid, 1, proto_ver=proto_ver)

    mid = 2
    publish_packet = dimq_test.gen_publish("bridge/disconnect/test", qos=1, mid=mid, payload="disconnect-message", proto_ver=proto_ver)
    publish_dup_packet = dimq_test.gen_publish("bridge/disconnect/test", qos=1, mid=mid, payload="disconnect-message", dup=True, proto_ver=proto_ver)
    ...
    try:
        ...
        mid = 128
        helper_publish_packet = dimq_test.gen_publish("bridge/disconnect/test", qos=1, mid=mid, payload="disconnect-message", proto_ver=proto_ver)
        helper_puback_packet = dimq_test.gen_puback(mid, proto_ver=proto_ver)
        ...
        dimq_test.do_send_receive(helper_sock, publish_packet, puback_packet, "helper puback")
        helper_sock.close()
        # End helper
        ...
        dimq_test.expect_packet(bridge, "2nd connect", connect_packet)
        bridge.send(connack_packet)
