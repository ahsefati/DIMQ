...
def do_test(unsubscribe_packet, reason_code, error_string):
    ...
    dimq_test.do_send_receive(sock, unsubscribe_packet, disconnect_packet, error_string=error_string)
    rc = 0
...
try:
    # mid == 0
    unsubscribe_packet = dimq_test.gen_unsubscribe(topic="test/topic", mid=0, proto_ver=5)
    do_test(unsubscribe_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "mid == 0")

    # command flags != 0x02
    unsubscribe_packet = dimq_test.gen_unsubscribe(topic="test/topic", mid=1, proto_ver=5, cmd=160)
    ...
    unsubscribe_packet = dimq_test.gen_unsubscribe(topic="test/topic", mid=1, proto_ver=5, properties=props)
    ...
    # Bad topic
    unsubscribe_packet = dimq_test.gen_unsubscribe(topic="#/test/topic", mid=1, proto_ver=5)
