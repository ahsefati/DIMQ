...
def do_test(allow_anonymous, password_file, username, expect_success):
    ...
    try:
        for proto_ver in [4, 5]:
            ...
            keepalive = 10
            if username:
                connect_packet = dimq_test.gen_connect("connect-test-%d" % (proto_ver), keepalive=keepalive, proto_ver=proto_ver, username="user", password="password")
            else:
                connect_packet = dimq_test.gen_connect("connect-test-%d" % (proto_ver), keepalive=keepalive, proto_ver=proto_ver)

            if proto_ver == 5:
                if expect_success == True:
                    connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)
                else:
                    connack_packet = dimq_test.gen_connack(rc=mqtt5_rc.MQTT_RC_NOT_AUTHORIZED, proto_ver=proto_ver, properties=None)
            else:
                if expect_success == True:
                    connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)
                else:
                    connack_packet = dimq_test.gen_connack(rc=5, proto_ver=proto_ver)
