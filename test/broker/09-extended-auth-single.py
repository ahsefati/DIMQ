...
connect1_packet = dimq_test.gen_connect("client-params-test1", keepalive=42, proto_ver=5, properties=props)
...
connack2_packet = dimq_test.gen_connack(rc=mqtt5_rc.MQTT_RC_BAD_AUTHENTICATION_METHOD, proto_ver=5, properties=None)
...
connect3_packet = dimq_test.gen_connect("client-params-test3", keepalive=42, proto_ver=5, properties=props)
connack3_packet = dimq_test.gen_connack(rc=mqtt5_rc.MQTT_RC_NOT_AUTHORIZED, proto_ver=5, properties=None)

# Single step, matching method, success
props = mqtt5_props.gen_string_prop(mqtt5_props.PROP_AUTHENTICATION_METHOD, "single")
props += mqtt5_props.gen_string_prop(mqtt5_props.PROP_AUTHENTICATION_DATA, "data")
connect4_packet = dimq_test.gen_connect("client-params-test5", keepalive=42, proto_ver=5, properties=props)
props = mqtt5_props.gen_string_prop(mqtt5_props.PROP_AUTHENTICATION_METHOD, "single")
connack4_packet = dimq_test.gen_connack(rc=0, proto_ver=5, properties=props)
...
connect5_packet = dimq_test.gen_connect("client-params-test6", keepalive=42, proto_ver=5, properties=props)
props = mqtt5_props.gen_string_prop(mqtt5_props.PROP_AUTHENTICATION_METHOD, "mirror")
props += mqtt5_props.gen_string_prop(mqtt5_props.PROP_AUTHENTICATION_DATA, "atademos")
connack5_packet = dimq_test.gen_connack(rc=0, proto_ver=5, properties=props)


broker = dimq_test.start_broker(filename=os.path.basename(__file__), use_conf=True, port=port)

try:
    sock = dimq_test.do_client_connect(connect1_packet, b"", timeout=20, port=port)
    sock.close()

    sock = dimq_test.do_client_connect(connect2_packet, connack2_packet, timeout=20, port=port)
    sock.close()

    sock = dimq_test.do_client_connect(connect3_packet, connack3_packet, timeout=20, port=port)
    sock.close()

    sock = dimq_test.do_client_connect(connect4_packet, connack4_packet, timeout=20, port=port)
    sock.close()

    sock = dimq_test.do_client_connect(connect5_packet, connack5_packet, timeout=20, port=port)
    sock.close()
