...
class MsgSequence(object):
    ...
    def add_default_connect(self, proto_ver):
        self.add_send(dimq_test.gen_connect(self.name, keepalive=60, proto_ver=proto_ver))
        self.add_recv(dimq_test.gen_connack(rc=0, proto_ver=proto_ver), "default connack")
    ...
    def _publish_message(self, msg):
        sock = dimq_test.client_connect_only(hostname="localhost", port=1888, timeout=2)
        sock.send(dimq_test.gen_connect("helper", keepalive=60))
        dimq_test.expect_packet(sock, "connack", dimq_test.gen_connack(rc=0))

        m = msg.message
        if m['qos'] == 0:
            sock.send(dimq_test.gen_publish(topic=m['topic'], payload=m['payload']))
        elif m['qos'] == 1:
            sock.send(dimq_test.gen_publish(mid=1, qos=1, topic=m['topic'], payload=m['payload']))
            dimq_test.expect_packet(sock, "helper puback", dimq_test.gen_puback(mid=1))
        elif m['qos'] == 2:
            sock.send(dimq_test.gen_publish(mid=1, qos=2, topic=m['topic'], payload=m['payload']))
            dimq_test.expect_packet(sock, "helper pubrec", dimq_test.gen_pubrec(mid=1))
            sock.send(dimq_test.gen_pubrel(mid=1))
            dimq_test.expect_packet(sock, "helper pubcomp", dimq_test.gen_pubcomp(mid=1))
        sock.close()
    ...
    for seq in sorted(sequences):
        ...
        for g in test_file:
            ...
            for t in tests:
                ...
                total += 1
                try:
                    sock = dimq_test.client_connect_only(hostname=hostname, port=port, timeout=2)
                    ...
                    print("\033[31m" + tname + " failed: " + str(e) + "\033[0m")
                    rc = 1
                except dimq_test.TestError as e:
                    print("\033[31m" + tname + " failed: " + str(e) + "\033[0m")
                    rc = 1
...
broker = dimq_test.start_broker(filename=os.path.basename(__file__), port=port, nolog=True)
