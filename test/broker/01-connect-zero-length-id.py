...
def do_test(per_listener, proto_ver, clean_start, allow_zero, client_port, expect_fail):
    ...
    rc = 1
    keepalive = 10
    connect_packet = dimq_test.gen_connect("", keepalive=keepalive, proto_ver=proto_ver, clean_session=clean_start)
    if proto_ver == 4:
        if expect_fail == True:
        ...
        if expect_fail == True:
            connack_packet = dimq_test.gen_connack(rc=128, proto_ver=proto_ver, properties=None)
        else:
            props = mqtt5_props.gen_string_prop(mqtt5_props.PROP_ASSIGNED_CLIENT_IDENTIFIER, "auto-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
            connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver, properties=props)
    ...
    broker = dimq_test.start_broker(filename=os.path.basename(__file__), port=port1, use_conf=True)

    try:
        sock = dimq_test.do_client_connect(connect_packet, connack_packet, port=client_port)
        sock.close()
        rc = 0
