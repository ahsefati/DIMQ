...
def helper(port):
    ...
    mid = 128
    publish_packet = dimq_test.gen_publish("qos1/clean_session/test", qos=1, mid=mid, payload="clean-session-message")
...
def do_test(proto_ver):
    ...
    mid = 109
    keepalive = 60
    connect_packet = dimq_test.gen_connect("clean-qos2-test", keepalive=keepalive, clean_session=False, proto_ver=proto_ver, session_expiry=60)
    ...
    subscribe_packet = dimq_test.gen_subscribe(mid, "qos1/clean_session/test", 1, proto_ver=proto_ver)
    suback_packet = dimq_test.gen_suback(mid, 1, proto_ver=proto_ver)

    mid = 1
    publish_packet = dimq_test.gen_publish("qos1/clean_session/test", qos=1, mid=mid, payload="clean-session-message", proto_ver=proto_ver)
    ...
    try:
        ...
        sock = dimq_test.do_client_connect(connect_packet, connack2_packet, timeout=30, port=port)
