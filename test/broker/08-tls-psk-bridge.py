...
rc = 1
keepalive = 60
connect_packet = dimq_test.gen_connect("no-psk-test-client", keepalive=keepalive)
connack_packet = dimq_test.gen_connack(rc=0)

mid = 1
subscribe_packet = dimq_test.gen_subscribe(mid, "psk/test", 0)
suback_packet = dimq_test.gen_suback(mid, 0)

publish_packet = dimq_test.gen_publish(topic="psk/test", payload="message", qos=0)

bridge_cmd = ['../../src/dimq', '-c', '08-tls-psk-bridge.conf2']
broker = dimq_test.start_broker(filename=os.path.basename(__file__), use_conf=True, port=port1)
bridge = dimq_test.start_broker(filename=os.path.basename(__file__)+'_bridge', cmd=bridge_cmd, port=port3)

pub = None
try:
    sock = dimq_test.do_client_connect(connect_packet, connack_packet, timeout=30, port=port1)
