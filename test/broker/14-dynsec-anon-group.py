...
# Client
connect_packet = dimq_test.gen_connect("cid", keepalive=keepalive, proto_ver=5)
connack_packet = dimq_test.gen_connack(rc=0, proto_ver=5)

mid = 1
subscribe_packet = dimq_test.gen_subscribe(mid, "anon/topic", qos=1, proto_ver=5)
suback_packet_fail = dimq_test.gen_suback(mid, mqtt5_rc.MQTT_RC_NOT_AUTHORIZED, proto_ver=5)
suback_packet_success = dimq_test.gen_suback(mid, 1, proto_ver=5)

disconnect_packet_kick = dimq_test.gen_disconnect(reason_code=mqtt5_rc.MQTT_RC_ADMINISTRATIVE_ACTION, proto_ver=5)
...
try:
    ...
    dimq_test.do_send_receive(sock, subscribe_packet_admin, suback_packet_admin, "suback admin")
    ...
    dimq_test.do_send_receive(csock, subscribe_packet, suback_packet_fail, "suback 1")
    ...
    dimq_test.expect_packet(csock, "disconnect 1", disconnect_packet_kick)
    csock.close()

    # Reconnect, subscribe should still fail
    csock = dimq_test.do_client_connect(connect_packet, connack_packet, timeout=5, port=port)
    dimq_test.do_send_receive(csock, subscribe_packet, suback_packet_fail, "suback 2")
    ...
    # Anon group is changed, so we are kicked
    dimq_test.expect_packet(csock, "disconnect 2", disconnect_packet_kick)
    csock.close()

    # Reconnect, subscribe should now succeed
    csock = dimq_test.do_client_connect(connect_packet, connack_packet, timeout=5, port=port)
    dimq_test.do_send_receive(csock, subscribe_packet, suback_packet_success, "suback 3")
