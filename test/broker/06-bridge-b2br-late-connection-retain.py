...
def do_test(proto_ver):
    ...
    c_connect_packet = dimq_test.gen_connect("client", keepalive=keepalive, proto_ver=proto_ver)
    c_connack_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)

    mid = 1
    publish_packet = dimq_test.gen_publish("bridge/test", qos=1, mid=mid, payload="message", retain=True, proto_ver=proto_ver)

    if proto_ver == 5:
        puback_packet = dimq_test.gen_puback(mid, proto_ver=proto_ver, reason_code=16)
    else:
    ...
    try:
        ...
        client = dimq_test.do_client_connect(c_connect_packet, c_connack_packet, timeout=20, port=port2)
        dimq_test.do_send_receive(client, publish_packet, puback_packet, "puback")
        client.close()
