...
def helper(topic, reason_code):
    ...
    publish_packet = dimq_test.gen_publish(topic, qos=1, mid=mid, payload="message", proto_ver=5)
    if reason_code == 0:
        puback_packet = dimq_test.gen_puback(mid, proto_ver=5)
    else:
        puback_packet = dimq_test.gen_puback(mid, proto_ver=5, reason_code=reason_code)
    sock = dimq_test.do_client_connect(connect_packet, connack_packet, port=port)
    dimq_test.do_send_receive(sock, publish_packet, puback_packet, "puback%d"%(mid))


rc = 1
keepalive = 60
connect_packet = dimq_test.gen_connect("pub-test", keepalive=keepalive, proto_ver=5)
