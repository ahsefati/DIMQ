...
def single_test(port, per_listener, username, topic, expect_deny):
    ...
    try:
        keepalive = 60
        connect_packet = dimq_test.gen_connect("acl-check", keepalive=keepalive, username=username)
        connack_packet = dimq_test.gen_connack(rc=0)

        mid = 1
        subscribe_packet = dimq_test.gen_subscribe(mid=mid, topic=topic, qos=1)
        suback_packet = dimq_test.gen_suback(mid=mid, qos=1)

        mid = 2
        publish1s_packet = dimq_test.gen_publish(topic=topic, mid=mid, qos=1, payload="message")
        puback1s_packet = dimq_test.gen_puback(mid)

        mid=1
        publish1r_packet = dimq_test.gen_publish(topic=topic, mid=mid, qos=1, payload="message")
        ...
        if expect_deny:
            dimq_test.expect_packet(sock, "puback", puback1s_packet)
            dimq_test.do_ping(sock)
        else:
            dimq_test.receive_unordered(sock, puback1s_packet, publish1r_packet, "puback / publish1r")
        sock.close()
