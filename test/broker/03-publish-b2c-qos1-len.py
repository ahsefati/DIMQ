...
def helper(port):
    ...
    mid = 1
    publish_packet = dimq_test.gen_publish("qos1/len/test", qos=1, mid=mid, payload="len-message")
...
def len_test(test, puback_packet):
    ...
    mid = 3265
    keepalive = 60
    connect_packet = dimq_test.gen_connect("pub-qos1-test", keepalive=keepalive, clean_session=False, proto_ver=5)
    connack_packet = dimq_test.gen_connack(flags=0, rc=0, proto_ver=5)

    subscribe_packet = dimq_test.gen_subscribe(mid, "qos1/len/test", 1, proto_ver=5)
    suback_packet = dimq_test.gen_suback(mid, 1, proto_ver=5)

    mid = 1
    publish_packet = dimq_test.gen_publish("qos1/len/test", qos=1, mid=mid, payload="len-message", proto_ver=5)
...
puback_packet = dimq_test.gen_puback(1)
len_test("qos1 len 2", puback_packet)

# Reason code, no properties
puback_packet = dimq_test.gen_puback(1, proto_ver=5, reason_code=0x00)
len_test("qos1 len 3", puback_packet)

# Reason code, empty properties
puback_packet = dimq_test.gen_puback(1, proto_ver=5, reason_code=0x00, properties="")
...
puback_packet = dimq_test.gen_puback(1, proto_ver=5, reason_code=0x00, properties=props)
