...
def do_test(proto_ver, clean_session1, clean_session2):
    ...
    mid = 1
    connect1_packet = dimq_test.gen_connect("will-helper", keepalive=keepalive, proto_ver=proto_ver)
    ...
    connect2_packet = dimq_test.gen_connect("will-test", keepalive=keepalive, proto_ver=proto_ver, will_topic="will/test", will_payload=b"LWT", clean_session=clean_session1, properties=connect_props1)
    connack2_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)

    connect3_packet = dimq_test.gen_connect("will-test", keepalive=keepalive, proto_ver=proto_ver, clean_session=clean_session2, properties=connect_props2)
    if clean_session1 == False and clean_session2 == False:
        connack3_packet = dimq_test.gen_connack(rc=0, flags=1, proto_ver=proto_ver)
    else:
        connack3_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)

    subscribe_packet = dimq_test.gen_subscribe(mid, "will/test", 0, proto_ver=proto_ver)
    suback_packet = dimq_test.gen_suback(mid, 0, proto_ver=proto_ver)

    publish_packet = dimq_test.gen_publish(topic="will/test", qos=0, payload="Client ready", proto_ver=proto_ver)
    publish_lwt_packet = dimq_test.gen_publish(topic="will/test", qos=0, payload="LWT", proto_ver=proto_ver)
    ...
    try:
        ...
        dimq_test.expect_packet(sock1, "publish 1", publish_packet)
        ...
        sock3 = dimq_test.do_client_connect(connect3_packet, connack3_packet, timeout=5, port=port)
        sock2.close()

        if clean_session1 == True or clean_session2 == True:
            dimq_test.expect_packet(sock1, "publish LWT", publish_lwt_packet)

        # Send the "ready" message again
        sock3.send(publish_packet)
        dimq_test.expect_packet(sock1, "publish 2", publish_packet)
