...
def do_test():
    ...
    connect_packet = dimq_test.gen_connect("subpub-qos2-test", keepalive=keepalive, proto_ver=5, properties=props)
    connack_packet = dimq_test.gen_connack(rc=0, proto_ver=5)

    mid = 1
    subscribe_packet = dimq_test.gen_subscribe(mid, "subpub/qos2", 2, proto_ver=5)
    suback_packet = dimq_test.gen_suback(mid, 2, proto_ver=5)

    mid = 1
    publish_packet1 = dimq_test.gen_publish("subpub/qos2", qos=2, mid=mid, payload="message1", proto_ver=5)
    pubrec_packet1 = dimq_test.gen_pubrec(mid, proto_ver=5)
    pubrel_packet1 = dimq_test.gen_pubrel(mid, proto_ver=5)
    pubcomp_packet1 = dimq_test.gen_pubcomp(mid, proto_ver=5)
    ...
    try:
        ...
        dimq_test.expect_packet(sock, "publish1", publish_packet1)
        dimq_test.do_send_receive(sock, pubrec_packet1, pubrel_packet1, "pubrel1")
        sock.send(pubcomp_packet1)

        dimq_test.expect_packet(sock, "publish2", publish_packet2)
        dimq_test.do_send_receive(sock, pubrec_packet2, pubrel_packet2, "pubrel2")
        sock.send(pubcomp_packet2)

        dimq_test.expect_packet(sock, "publish3", publish_packet3)
        dimq_test.do_send_receive(sock, pubrec_packet3, pubrel_packet3, "pubrel3")
