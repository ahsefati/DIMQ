...
def do_test(topic, succeeds):
    ...
    mid = 53
    keepalive = 60
    connect_packet = dimq_test.gen_connect("subpub-qos0-test", keepalive=keepalive)
    connack_packet = dimq_test.gen_connack(rc=0)

    subscribe_packet = dimq_test.gen_subscribe(mid, topic, 0)
    suback_packet = dimq_test.gen_suback(mid, 0)

    publish_packet = dimq_test.gen_publish(topic, qos=0, payload="message")
    ...
    try:
        ...
        if succeeds == True:
            ...
            dimq_test.do_send_receive(sock, publish_packet, publish_packet, "publish")
        else:
