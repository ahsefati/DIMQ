...
def do_test(proto_ver, per_listener, username):
    ...
    rc = 1
    keepalive = 60
    connect_packet = dimq_test.gen_connect("retain-check", keepalive=keepalive, username=username, proto_ver=proto_ver)
    ...
    connect2_packet = dimq_test.gen_connect("retain-recv", keepalive=keepalive, username=u, proto_ver=proto_ver)
    connack2_packet = dimq_test.gen_connack(rc=0, proto_ver=proto_ver)

    mid = 1
    publish_packet = dimq_test.gen_publish("test/topic", qos=0, payload="retained message", retain=True, proto_ver=proto_ver)
    subscribe_packet = dimq_test.gen_subscribe(mid, "test/topic", 0, proto_ver=proto_ver)
    ...
    try:
        ...
        sock = dimq_test.do_client_connect(connect2_packet, connack2_packet, port=port2)
        dimq_test.do_send_receive(sock, subscribe_packet, suback_packet, "suback 1")
        ...
        sock = dimq_test.do_client_connect(connect2_packet, connack2_packet, port=port2)
        dimq_test.do_send_receive(sock, subscribe_packet, suback_packet, "suback 2")
