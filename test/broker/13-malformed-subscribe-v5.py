...
def do_test(subscribe_packet, reason_code, error_string):
    ...
    mid = 0
    disconnect_packet = dimq_test.gen_disconnect(proto_ver=5, reason_code=reason_code)

    sock = dimq_test.do_client_connect(connect_packet, connack_packet, port=port)
    dimq_test.do_send_receive(sock, subscribe_packet, disconnect_packet, error_string=error_string)
    rc = 0
...
try:
    # mid == 0
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=1, mid=0, proto_ver=5)
    do_test(subscribe_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "mid == 0")

    # qos > 2
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=3, mid=1, proto_ver=5)
    do_test(subscribe_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "qos > 2")

    # retain handling = 0x30
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=0x30, mid=1, proto_ver=5)
    do_test(subscribe_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "retain handling = 0x30")

    # subscription options = 0xC0
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=0xC0, mid=1, proto_ver=5)
    do_test(subscribe_packet, mqtt5_rc.MQTT_RC_MALFORMED_PACKET, "subscription options = 0xC0")

    # command flags != 0x02
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=1, mid=1, proto_ver=5, cmd=128)
    ...
    subscribe_packet = dimq_test.gen_subscribe(topic="test/topic", qos=1, mid=1, proto_ver=5, properties=props)
    ...
    # Bad topic
    subscribe_packet = dimq_test.gen_subscribe(topic="#/test/topic", qos=1, mid=1, proto_ver=5)
