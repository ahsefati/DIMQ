...
def do_test(plugin_ver):
    ...
    rc = 1
    connect1_packet = dimq_test.gen_connect("acl-change-test", clean_session=False)
    connack1_packet = dimq_test.gen_connack(rc=0)

    connect2_packet = dimq_test.gen_connect("acl-change-test", clean_session=False)
    connack2_packet = dimq_test.gen_connack(rc=0,flags=1)

    mid = 1
    subscribe_packet = dimq_test.gen_subscribe(mid, "#", 0)
    suback_packet = dimq_test.gen_suback(mid, 0)

    mid = 2
    publish1_packet = dimq_test.gen_publish("publish/topic", qos=2, mid=mid, payload="message")
    ...
    try:
        ...
        dimq_test.do_send_receive(sock, publish1_packet, pubrec1_packet, "pubrec")
        sock.close()
        ...
        dimq_test.do_send_receive(sock, pubrel1_packet, pubcomp1_packet, "pubcomp")
