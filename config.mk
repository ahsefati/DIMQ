...
#
# Largely, these are options that are designed to make dimq run more
...
# dimq won't track heap memory usage nor export '$SYS/broker/heap/current
# size', but will use slightly less memory and CPU time.
WITH_MEMORY_TRACKING:=yes

# Compile with database upgrading support? If disabled, dimq won't
...
# Build with systemd support. If enabled, dimq will notify systemd after
...
# with the static libdimq library. This may be required on some systems
...
# Build dimq_sub with cJSON support
WITH_CJSON:=yes

# Build dimq with support for the $CONTROL topics.
WITH_CONTROL:=yes
...
ifeq ($(WITH_SHARED_LIBRARIES),yes)
	CLIENT_LDADD:=${CLIENT_LDADD} ../lib/libdimq.so.${SOVERSION}
endif
...
ifneq ($(UNAME),SunOS)
	LIB_LDFLAGS:=$(LIB_LDFLAGS) -Wl,--version-script=linker.version -Wl,-soname,libdimq.so.$(SOVERSION)
endif
